...
internal class BrowserToolbarMiddleware(initialDependencies: Dependencies) :
            ...
            is BrowserToolbarAction.Init -> {
                ...
                next(
                    BrowserToolbarAction.Init(
                        ...
                        editState = EditState(editActionsStart = listOfNotNull(buildSearchSelector())),
                    )
                )
    ...
    private fun buildStartBrowserActions() =
        listOf(
            ActionButtonRes(
    ...
    private fun buildStartPageActions() =
        listOf(
    ...
    private fun buildPageActionsEnd() =
        listOf(
            ActionButtonRes(
    ...
    private fun buildSearchSelector(): Action =
        SearchSelectorAction(
            icon = SearchSelectorIconRes(iconsR.drawable.mozac_ic_search_24),
            contentDescription = SearchSelectorDescriptionRes(R.string.search_selector_description),
            menu = {
        ...
        progressAnimationJob =
            CoroutineScope(Dispatchers.Main).launch {
                loop@ for (progress in PROGRESS_RANGE step RELOAD_STEP_SIZE) {
                    ...
                    store.dispatch(UpdateProgressBarConfig(buildProgressBar(progress)))
                }
            }
    }
