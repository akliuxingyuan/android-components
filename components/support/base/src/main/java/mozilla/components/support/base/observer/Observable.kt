...
/**
 * Interface for observables. This interface is implemented by ObserverRegistry so that classes that want to be
 * observable can implement the interface by delegation:
 *
 * <code> val registry = ObserverRegistry<MyObserverInterface>()
 *
...
interface Observable<T> {
     ...
     * Registers an observer to get notified about changes.
     *
     * The observer will only be notified if the view is attached and will be unregistered/ registered if the attached
     * state changes.
     *
     * @param observer the observer to register.
    ...
    /**
     * Notifies all registered observers about a change. If there is no observer the notification is queued and sent to
     * the first observer that is registered.
     *
     * @param block the notification (method on the observer to be invoked).
     */
    ...
    /**
     * Resumes the provided observer. Notifications sent since it was last paused (see [pauseObserver]]) are lost and
...
/**
 * A deprecated version of [Observable] to migrate and deprecate existing components individually. All components
 * implement [Observable] by delegate so this makes it easy to deprecate without having to override all methods in each
 * component separately.
 */
@Deprecated(OBSERVER_DEPRECATION_MESSAGE)
interface DeprecatedObservable<T> : Observable<T> {

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE) override fun register(observer: T)

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE)
    override fun register(observer: T, owner: LifecycleOwner, autoPause: Boolean)

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE) override fun register(observer: T, view: View)

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE) override fun unregister(observer: T)

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE) override fun unregisterObservers()

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE) override fun notifyObservers(block: T.() -> Unit)

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE) override fun notifyAtLeastOneObserver(block: T.() -> Unit)

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE) override fun pauseObserver(observer: T)

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE) override fun resumeObserver(observer: T)

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE)
    override fun <R> wrapConsumers(block: T.(R) -> Boolean): List<(R) -> Boolean>

    @Deprecated(OBSERVER_DEPRECATION_MESSAGE) override fun isObserved(): Boolean
}
