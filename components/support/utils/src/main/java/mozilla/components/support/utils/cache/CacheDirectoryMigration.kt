...
/**
 * Moves a legacy cache directory into a persistent app-owned directory once per source/target pair. The migration will
 * be applied only if the target directory doesn't exist, meaning that it is intended to run only once, when user
 * updates from a version using the legacy cache directory to a version using the new directory. Clearing cache won't
 * trigger a new migration, as the migration is only necessary if we have files in the legacy cache, which need to be
 * migrated to the new cache directory.
 *
