 ...
 *
 * This function prevents "double-decoding" or corruption by checking for malformed percent-encoding. If the string
 * contains a '%' character that is not followed by two valid hexadecimal digits (e.g., "test%.txt" or "discount 10%"),
 * it is treated as already decoded, and the original string is returned.
 *
 * @return The decoded string if it contains valid percent-encoding; otherwise, the original string.
 */
