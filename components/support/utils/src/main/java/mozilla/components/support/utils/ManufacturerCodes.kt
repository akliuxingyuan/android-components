...
/**
 * Used to check if a device is from a specific manufacturer, using the value returned by
...
internal object ManufacturerCodes {
    ...
    internal val isHuawei
        get() = manufacturer.equals(HUAWEI, ignoreCase = true)

    internal val isSamsung
        get() = manufacturer.equals(SAMSUNG, ignoreCase = true)

    internal val isXiaomi
        get() = manufacturer.equals(XIAOMI, ignoreCase = true)

    internal val isOnePlus
        get() = manufacturer.equals(ONE_PLUS, ignoreCase = true)

    internal val isLG
        get() = manufacturer.equals(LG, ignoreCase = true)
...
/**
 * A concrete implementation of [ManufacturerChecker] that delegates manufacturer checks to the [ManufacturerCodes]
 * object, which in turn uses [android.os.Build.MANUFACTURER].
 */
