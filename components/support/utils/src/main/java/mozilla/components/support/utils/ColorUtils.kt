...
object ColorUtils {
    ...
    @JvmStatic
    @ColorInt
    fun getSecondaryReadableTextColor(@ColorInt backgroundColor: Int): Int {
     ...
     *
     * @param factor How much lighter the new color should be. A higher value will produce a lighter color.
     */
    fun ComposeColor.lighten(@FloatRange(from = 0.0, to = 1.0) factor: Float) = lerp(this, ComposeColor.White, factor)

    /**
     * Produce a darker color than this using the given factor.
     *
     * @param factor How much darken the new color should be. A higher value will produce a darker color.
     */
    fun ComposeColor.darken(@FloatRange(from = 0.0, to = 1.0) factor: Float) = lerp(this, ComposeColor.Black, factor)
}
