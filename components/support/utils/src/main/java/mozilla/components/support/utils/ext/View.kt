...
/**
 * Helper method to allow catching all back button presses and back gesture events. On Android 33+ this will also catch
 * the back press / gesture events that would close the keyboard.
 *
 * This will start observing back presses / back gesture events right away and will handle the automatic observers
 * cleanup whenever the view is destroyed.
 *
 * @param onBack The action to invoke for back presses or back gestures.
 */
fun View.handleBackEvents(onBack: () -> Unit) {
    val backInvokedDispatcher =
        if (SDK_INT >= TIRAMISU) {
            context.getActivityWindow()?.onBackInvokedDispatcher
        } else {
            null
        }
    val backInvokedCallback =
        if (SDK_INT >= TIRAMISU) {
    ...
    val backPressedDispatcher: OnBackPressedDispatcher? =
        (findViewTreeOnBackPressedDispatcherOwner() ?: findBackPressedDispatcherOwner(context))?.onBackPressedDispatcher
...
/**
 * Fallback to try finding the [OnBackPressedDispatcherOwner] in any of [context] ancestors. Functionality is mimicking
 * the Jetpack Compose functionality from the `BackHandler` functionality.
 */
 ...
 *
 * Inspired by https://stackoverflow.com/questions/2150078/how-to-check-visibility-of-software-keyboard-in-android API
 * 30 adds a native method for this. We should use it (and a compat method if one is added) when it becomes available
 */
...
/**
 * A safer version of [ViewCompat.getRootWindowInsets] that does not throw a NullPointerException if the view is not
 * attached.
 */
...
@VisibleForTesting
internal fun View.getWindowVisibleDisplayFrame(): Rect =
    with(Rect()) {
