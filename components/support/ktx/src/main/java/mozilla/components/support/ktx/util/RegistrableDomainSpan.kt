...
/**
 * Determines the start and end indexes of either the registrable domain or the full host within a URL string and mark
 * these with [RegistrableDomainSpan].
 *
 * @param cleanURL The complete URL to analyze.
 * @param publicSuffixList The [PublicSuffixList] used to identify the URL host.
 * @return The same [cleanURL] received but with the registrable domain marked with [RegistrableDomainSpan] if this
 *   could be identified in the given [cleanURL].
 */
 ...
 *
 * @param publicSuffixList The [PublicSuffixList] used to identify the URL host.
 * @return A Pair of (startIndex, endIndex) for the registrable domain within the host, or null if the host is an IP
 *   address or no registrable domain could be found.
 */
private suspend fun String.getRegistrableDomainIndexRangeInHost(publicSuffixList: PublicSuffixList): Pair<Int, Int>? {
    if (isIpv4OrIpv6()) return null

    val normalizedHost = removeSuffix(".")

    val registrableDomain = publicSuffixList.getPublicSuffixPlusOne(normalizedHost).await() ?: return null
