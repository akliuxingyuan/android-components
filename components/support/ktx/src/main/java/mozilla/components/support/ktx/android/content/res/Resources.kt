...
/**
 * Returns the character sequence associated with a given resource [id], substituting format arguments with additional
 * styling spans.
 *
 ...
 * @param spanParts The format arguments that will be used for substitution. The first element of each pair is the text
 *   to insert, similar to [String.format]. The second element of each pair is a span that will be used to style the
 *   inserted string.
 */
...
/** [Appendable] implementation that wraps [SpannableStringBuilder] and inserts spans from the span parts array. */
private class SpannableAppendable(
    ...
    private val spansMap = spanParts.toMap().mapKeys { (key) -> key.let { it as? CharSequence ?: it.toString() } }
    ...
    /**
     * Tries to find [csq] in the [spansMap] and use the corresponding span value. If [csq] is not found, the map is
     * searched manually by converting values to strings. If no match is found afterwards, [csq] is appended with no
     * corresponding span.
     */
