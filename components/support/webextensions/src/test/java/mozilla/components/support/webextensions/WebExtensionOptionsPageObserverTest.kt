...
class WebExtensionOptionsPageObserverTest {
    ...
    @Test
    fun `observes and forwards request to open options page`() =
    ...
    @Test
    fun `GIVEN options page session exists WHEN the same extension requests again THEN do not call onOpenOptionsPage again`() =
        runTest(testDispatcher) {
            val extensionId = "ext"
            val store = BrowserStore(BrowserState(extensions = mapOf(extensionId to WebExtensionState(extensionId))))
    ...
    @Test
    fun `GIVEN options page session exists WHEN a different extension requests THEN do not call onOpenOptionsPage again`() =
        runTest(testDispatcher) {
            ...
            val store =
                BrowserStore(
                    BrowserState(
                        extensions =
                            mapOf(
                                extensionId1 to WebExtensionState(extensionId1),
                                extensionId2 to WebExtensionState(extensionId2),
                            )
                    )
                )
