...
class LicenseDetector : Detector(), SourceCodeScanner {
    ...
    companion object {
        ...
        val ISSUE_MISSING_LICENSE =
            Issue.create(
                id = "MissingLicense",
                briefDescription = "File doesn't start with the license comment",
                explanation =
                    "Every file must start with the license comment:\n" +
        ...
        val ISSUE_INVALID_LICENSE_FORMAT =
            Issue.create(
    ...
    override fun createUastHandler(context: JavaContext): UElementHandler? = LicenseCommentChecker(context)
}
