...
class ContextCompatDetector : Detector(), SourceCodeScanner {
    ...
    companion object {
        ...
        val ISSUE_GET_DRAWABLE_CALL =
            Issue.create(
                id = "UnsafeCompatGetDrawable",
                briefDescription = "Prohibits using the ContextCompat.getDrawable method",
                explanation =
                    "Using this method can lead to crashes in older Android versions as newer features might not be available",
                category = Category.CORRECTNESS,
                severity = Severity.ERROR,
                implementation = Implementation,
            )

        val ISSUE_GET_COLOR_STATE_LIST_CALL =
            Issue.create(
