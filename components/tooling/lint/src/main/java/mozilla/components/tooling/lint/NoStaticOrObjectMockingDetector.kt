 ...
 *
 * This detector aims to discourage practices that can lead to brittle and hard-to-maintain tests. Static mocking and
 * object mocking often indicate a design that could be improved for better testability, for example, by using
 * dependency injection.
 *
 * It checks for specific methods from popular mocking libraries like MockK and Mockito:
 * - **MockK:** `mockkStatic`, `unmockkStatic`, `clearStaticMock`, `mockkObject`, `unmockkObject`, `clearObjectMock`
 * - **Mockito:** `mockStatic`, `mockConstruction`
 *
 * When such methods are detected, it reports an issue suggesting refactoring the code to avoid these types of mocking.
 *
...
class NoStaticOrObjectMockingDetector : Detector(), Detector.UastScanner {
    ...
    companion object {
        ...
        @JvmField
        val ISSUE_NO_STATIC_MOCKING: Issue =
            Issue.create(
                id = "NoStaticMocking",
        ...
        @JvmField
        val ISSUE_NO_OBJECT_MOCKING: Issue =
            Issue.create(
                id = "NoObjectMocking",
    ...
    override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
        ...
        // --- Mockito Checks ---
        if (
            evaluator.isMemberInClass(method, "org.mockito.Mockito") &&
