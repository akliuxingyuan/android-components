...
class LicenseCommentChecker(private val context: JavaContext) : UElementHandler() {
    ...
    override fun visitFile(node: UFile) {
        if (!node.sourcePsi.startsWithComment()) {
            ...
            if (firstComment.text != ValidLicenseForKotlinFiles) {
                ...
                val nextSibling = firstComment.sourcePsi.siblings(withItself = false).firstOrNull()
                if (nextSibling?.text != "\n\n") {
    ...
    private fun reportMissingLicense(node: UFile) =
        context.report(
    ...
    private fun reportInvalidLicenseFormat(comment: UComment) =
        context.report(
    ...
    private fun reportMissingLeadingNewLineCharacter(licenseComment: UComment) =
        context.report(
            LicenseDetector.ISSUE_INVALID_LICENSE_FORMAT,
    ...
    private fun addLicenseQuickFix() =
        LintFix.create()
    ...
    private fun replaceCommentWithValidLicenseFix(comment: UComment) =
        LintFix.create()
    ...
    private fun addLeadingNewLineQuickFix(licenseComment: UComment) =
        LintFix.create()
