...
private val FRAMEWORK_ELEMENTS =
    setOf(
 ...
 *
 * This data was manually added from the Type Hierarchy IDE action. Last update: 8/5/2020. In the future, we could
 * generate this list statically to prevent transcription errors and errors from refactors.
 */
private val CUSTOM_VIEW_ELEMENTS =
    setOf(
...
private const val FRAMEWORK_ISSUE_MESSAGE =
...
private val FRAMEWORK_ISSUE_EXPLANATION =
    """
...
private val CUSTOM_VIEW_ISSUE_MESSAGE =
    "Custom views extending `ConstraintLayout` are less " +
        "efficient because they cannot share other `ConstraintLayout` defined in file."
private val CUSTOM_VIEW_ISSUE_EXPLANATION =
    """
    `ConstraintLayout` is slow to inflate so we
...
/**
 * An issue where multiple ConstraintLayouts that are defined by the framework are detected in the same file. This is
 * distinct from [CUSTOM_VIEW_ISSUE] because that one handles custom views. We don't want to combine them because
 * solving a custom view ConstraintLayout problem rarely has a clear cut solution and we don't want to fail the build on
 * checks that aren't clear cut.
 */
private val FRAMEWORK_ISSUE =
    Issue.create(
...
/**
 * An issue where multiple ConstraintLayouts, that may also be defined by us as custom views, are detected in the same
 * file. See [FRAMEWORK_ISSUE] for why these are distinct.
 */
private val CUSTOM_VIEW_ISSUE =
    Issue.create(
 ...
 * - if we add additional views that extend ConstraintLayout, this check won't know about them (though we could generate
 *   this information and include it in this check via static analysis)
 */
