 ...
 *
 * This detector aims to enforce the use of the custom `displaySnackbar` function provided in
 * `mozilla.components.compose.base.snackbar`. The custom function includes additional accessibility handling that might
 * be missed when using the Material 3 function directly.
 *
 * It scans for method calls named "showSnackbar" and checks if the receiver of the call is an instance of
 * `androidx.compose.material3.SnackbarHostState` or its subclasses. If such a usage is found, it reports an error,
 * guiding the developer to use the preferred wrapper.
 */
class ShowSnackbarDetector : Detector(), SourceCodeScanner {
     ...
     *
     * This object holds constants and the [Issue] definition for the lint rule. The rule flags direct usage of
     * `SnackbarHostState.showSnackbar(...)` and suggests using `displaySnackbar` from
     * `mozilla.components.compose.base.snackbar` for better accessibility.
     */
    companion object {
        ...
        @JvmField
        val ISSUE_NO_DIRECT_SHOW_SNACKBAR: Issue =
            Issue.create(
