...
class SponsoredContentsDaoTest {
    ...
    @Test
    fun `WHEN a sponsored content is inserted and fetched THEN the newly inserted sponsored content is returned`() =
        runTest {
    ...
    @Test
    fun `GIVEN a sponsored content is persisted in the database WHEN the sponsored content is deleted THEN sponsored content is removed from the database`() =
        runTest {
            val sponsoredContent1 = PocketTestResources.sponsoredContentEntity
            val sponsoredContent2 = PocketTestResources.sponsoredContentEntity.copy(url = sponsoredContent1.url + "2")
    ...
    @Test
    fun `WHEN all sponsored contents are deleted THEN all sponsored contents are removed from the database`() =
        runTest {
            val sponsoredContent1 = PocketTestResources.sponsoredContentEntity
            val sponsoredContent2 = PocketTestResources.sponsoredContentEntity.copy(url = sponsoredContent1.url + "2")
    ...
    @Test
    fun `GIVEN sponsored contents are persisted in the database WHEN new sponsored contents are fetched and a clean and insert happens THEN new list of sponsored contents are persisted in the database`() =
        runTest {
            setupDatabaseForTransactions()
            val sponsoredContent1 = PocketTestResources.sponsoredContentEntity
            val sponsoredContent2 = PocketTestResources.sponsoredContentEntity.copy(url = sponsoredContent1.url + "2")
            val sponsoredContent3 = PocketTestResources.sponsoredContentEntity.copy(url = sponsoredContent1.url + "3")
            val sponsoredContent4 = PocketTestResources.sponsoredContentEntity.copy(url = sponsoredContent1.url + "4")
    ...
    @Test
    fun `GIVEN no sponsored contents are persisted in the database WHEN sponsored content impression is recorded THEN don't persist any data in the database`() =
        runTest {
    ...
    @Test
    fun `GIVEN sponsored contents are persisted in the database WHEN sponsored content impressions are recorded THEN persist impressions only for existing sponsored content`() =
        runTest {
            setupDatabaseForTransactions()
            val sponsoredContent1 = PocketTestResources.sponsoredContentEntity
            val sponsoredContent2 = PocketTestResources.sponsoredContentEntity.copy(url = sponsoredContent1.url + "2")
