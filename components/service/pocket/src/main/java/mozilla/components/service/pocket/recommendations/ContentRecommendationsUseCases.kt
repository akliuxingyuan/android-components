...
internal class ContentRecommendationsUseCases(
    ...
    internal inner class GetContentRecommendations(
        @get:VisibleForTesting internal val context: Context = this@ContentRecommendationsUseCases.appContext
    ) {
    ...
    internal inner class FetchContentRecommendations(
        @get:VisibleForTesting internal val context: Context = this@ContentRecommendationsUseCases.appContext,
        @get:VisibleForTesting internal val client: Client = this@ContentRecommendationsUseCases.client,
        ...
        suspend operator fun invoke(): Boolean {
            ...
            getContentRecommendationsRepository(context).updateContentRecommendations(response.data)
    ...
    /**
     * Use case for updating the number of impressions (times shown) for a list of content recommendations.
     *
     * @param context Application [Context]. Prefer sending application context to limit the possibility of even small
     *   leaks.
     */
    internal inner class UpdateRecommendationsImpressions(
        ...
        /**
         * Updates the number of impressions (times shown) for the provided list of [ContentRecommendation]s.
         *
         * @param recommendationsShown The list of [ContentRecommendation]s that have an updated impressions.
         */
        suspend operator fun invoke(recommendationsShown: List<ContentRecommendation>) {
            if (recommendationsShown.isEmpty()) return

            getContentRecommendationsRepository(context).updateContentRecommendationsImpressions(recommendationsShown)
        }
    }
