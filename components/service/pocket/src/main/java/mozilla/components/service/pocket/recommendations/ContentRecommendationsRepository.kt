...
internal class ContentRecommendationsRepository(context: Context) {
    ...
    @VisibleForTesting internal val contentRecommendationsDao by lazy { database.value.contentRecommendationsDao() }

    /**
     * Returns all the [ContentRecommendation]s that are persisted in storage.
     *
     * @return the list of [ContentRecommendation]s that is stored in the content recommendation storage.
     */
    suspend fun getContentRecommendations(): List<ContentRecommendation> {
        return contentRecommendationsDao.getContentRecommendations().map { it.toContentRecommendation() }
    }

    /**
     * Updates the number of impressions (times shown) for the provided list of [ContentRecommendation]s that are
     * persisted in storage.
     *
     * @param recommendationsShown The list of [ContentRecommendation]s to update.
     */
    suspend fun updateContentRecommendationsImpressions(recommendationsShown: List<ContentRecommendation>) {
    ...
    /**
     * Adds the provided [ContentRecommendationsResponse] to storage updating and replacing the content recommendations
     * in storage.
     *
