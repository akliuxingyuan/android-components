...
class FxRelayTest {
    ...
    @Test
    fun `GIVEN profile fetch throws API error WHEN fetchAccountDetails THEN returns null`() = runTest {
        val fxRelay =
            createFxRelay(
    ...
    fun `GIVEN network error WHEN fetchAccountDetails THEN returns null`() = runTest {
        val fxRelay = createFxRelay(fetchProfileException = RelayApiException.Network("timeout"))
        assertNull(fxRelay.fetchAccountDetails())
    }

    // fetchEmailMasks tests
    @Test
    fun `GIVEN valid token and addresses WHEN fetchEmailMasks THEN returns list of email masks`() = runTest {
        val fxRelay = createFxRelay(addresses = listOf(fakeAddress(fullAddress = "test@relay.firefox.com")))
        assertEquals(
    ...
    fun `GIVEN network error WHEN fetchEmailMasks THEN returns null`() = runTest {
        val fxRelay = createFxRelay(fetchAddressesException = RelayApiException.Network("timeout"))
    ...
    fun `GIVEN API error WHEN fetchEmailMasks THEN returns null`() = runTest {
        val fxRelay =
            createFxRelay(fetchAddressesException = RelayApiException.Api(500u, "server_error", "Internal error"))
        assertNull(fxRelay.fetchEmailMasks())
    }

    @Test
    fun `GIVEN other error WHEN fetchEmailMasks THEN returns null`() = runTest {
        val fxRelay = createFxRelay(fetchAddressesException = RelayApiException.Other("unexpected error"))
        assertNull(fxRelay.fetchEmailMasks())
    }

    @Test
    fun `GIVEN multiple addresses WHEN fetchEmailMasks THEN returns all masks mapped correctly`() = runTest {
        val fxRelay =
            createFxRelay(
                addresses =
                    listOf(
    ...
    @Test
    fun `GIVEN valid token WHEN createEmailMask THEN returns mask with GENERATED source`() = runTest {
        val fxRelay = createFxRelay(createAddressResult = fakeAddress(fullAddress = "new@relay.firefox.com"))
    ...
    @Test
    fun `GIVEN free tier limit reached and no existing masks WHEN createEmailMask THEN returns null`() = runTest {
        val fxRelay =
            createFxRelay(
                addresses = emptyList(),
                createAddressException =
    ...
    fun `GIVEN non-free_tier_limit API error WHEN createEmailMask THEN returns null`() = runTest {
        val fxRelay =
            createFxRelay(createAddressException = RelayApiException.Api(500u, "server_error", "Internal error"))
        assertNull(fxRelay.createEmailMask())
    }

    @Test
    fun `GIVEN network error WHEN createEmailMask THEN returns null`() = runTest {
        val fxRelay = createFxRelay(createAddressException = RelayApiException.Network("no connection"))
        assertNull(fxRelay.createEmailMask())
    }

    @Test
    fun `GIVEN Other error WHEN createEmailMask THEN returns null`() = runTest {
        val fxRelay = createFxRelay(createAddressException = RelayApiException.Other("unexpected error"))
        assertNull(fxRelay.createEmailMask())
    }
    ...
    private fun fakeAddress(fullAddress: String = "fake@relay.firefox.com") =
        RelayAddress(
            maskType = "random",
            enabled = true,
            description = "",
