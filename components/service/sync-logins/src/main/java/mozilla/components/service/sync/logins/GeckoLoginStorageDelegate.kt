 ...
 *
 * An abstraction that handles the persistence and retrieval of [LoginEntry]s so that Gecko doesn't have to.
 *
 * In order to use this class, attach it to the active [GeckoRuntime] as its `loginStorageDelegate`. It is not designed
 * to work with other engines.
 *
 ...
 *     - ([GeckoLoginStorageDelegate] is not involved with this step) `SaveLoginDialogFragment` is shown to the user,
 *       who decides whether or not to save the [LoginEntry] and gives them a chance to manually adjust the
 *       username/password fields.
 *         - `SaveLoginDialogFragment` uses `DefaultLoginValidationDelegate` to determine what the result of the
 *           operation will be: saving a new login, updating an existing login, or filling in a blank username.
