...
object GlobalLoginsDependencyProvider {
    ...
    @VisibleForTesting internal var loginsStorage: Lazy<LoginsStorage>? = null

    /**
     * Initializes logins storage for running the maintenance task via [SyncableLoginsStorageWorker]. This method should
     * be called in client application's onCreate method and before
     * [SyncableLoginsStorage.registerStorageMaintenanceWorker] in order to run the worker while the app is not running.
     */
    ...
    /**
     * Provides [LoginsStorage] globally when needed for [SyncableLoginsStorageWorker] to run maintenance on the
     * storage.
     */
