...
/**
 * Thrown if the application attempts to complete an OAuth flow when no OAuth flow has been initiated. This may indicate
 * a user who navigated directly to the OAuth `redirect_uri` for the application.
 */
typealias FxaNoExistingAuthFlow = mozilla.appservices.fxaclient.FxaException.NoExistingAuthFlow

/**
 * Thrown when the Rust library hits an unexpected error that isn't a panic. This may indicate library misuse, network
 * errors, etc.
 */
...
fun FxaException.shouldPropagate(): Boolean {
    return when (this) {
        ...
        is FxaNoExistingAuthFlow -> false
...
sealed class AccountManagerException(message: String) : Exception(message) {
     ...
     */
    class AuthRecoveryCircuitBreakerException(operation: String) :
        AccountManagerException("Auth recovery circuit breaker triggered by: $operation")
     ...
     */
    class MissingKeyFromSyncScopedAccessToken(operation: String) :
        AccountManagerException("Encountered an access token without a key: $operation")

    /** Failure when running side effects to complete the authentication process. */
    class AuthenticationSideEffectsFailed :
        AccountManagerException("Failure when running side effects to complete authentication")
}
...
sealed class SendCommandException : Exception {
    ...
    /**
     * An exception thrown when one or more [DeviceCommandOutgoing.CloseTab.urls] couldn't be sent.
     *
     * The caller should back off and retry sending the [urls] in a new [DeviceCommandOutgoing.CloseTab] command.
     */
    class TabsNotClosed(val urls: List<String>) : SendCommandException("Couldn't send all URLs in close tabs command")

    /** An exception used for [CrashReporting] when a command couldn't be sent for any other reason. */
