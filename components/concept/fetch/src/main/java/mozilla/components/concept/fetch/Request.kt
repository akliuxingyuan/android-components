 ...
 *   connection can be established, a [java.net.SocketTimeoutException] is raised. A timeout of zero is interpreted as
 *   an infinite timeout.
 ...
 * @property cookiePolicy A policy to specify whether or not cookies should be sent with the request, defaults to
 *   [CookiePolicy.INCLUDE]
 * @property useCaches Whether caches should be used or a network request should be forced, defaults to true (use
 *   caches).
 * @property private Whether the request should be performed in a private context, defaults to false. The feature is not
 *   support in all [Client]s, check support before using.
 * @property useOhttp Whether the request should be performed using the OHTTP library, defaults to false. Currently only
 *   GeckoViewFetchClient supports it.
 * @see [Headers.Names]
 * @see [Headers.Values]
 */
data class Request(
     ...
     */
    class Body(private val stream: InputStream) : Closeable {
        companion object {
             ...
             * "application/x-www-form-urlencoded". Parameters are formatted as "key1=value1&key2=value2..." and values
             * are percent-encoded. If the given map is empty, the response body will contain the empty string.
             *
     ...
     * Request methods.
     *
     * The request method token is the primary source of request semantics; it indicates the purpose for which the
     * client has made this request and what is expected by the client as a successful result.
     *
