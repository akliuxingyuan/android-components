...
interface RequestInterceptor {
    ...
    /** An alternative response for an error request. Used to load an encoded URI directly. */
     ...
     * @return An [InterceptionResponse] object containing alternative content or an alternative URL. Null if the
     *   original request should continue to be loaded.
     */
     ...
     * @return An [ErrorResponse] object containing content to display for the provided error type.
     */
    fun onErrorRequest(session: EngineSession, errorType: ErrorType, uri: String?): ErrorResponse? = null

    /**
     * Returns whether or not this [RequestInterceptor] should intercept load requests initiated by the app (via direct
     * calls to [EngineSession.loadUrl]). All other requests triggered by users interacting with web content (e.g.
     * following links) or redirects will always be intercepted.
     *
     * @return true if app initiated requests should be intercepted, otherwise false. Defaults to false.
     */
