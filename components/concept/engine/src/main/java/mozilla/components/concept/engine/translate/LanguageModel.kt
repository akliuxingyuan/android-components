 ...
 *
 * Please note, a single LanguageModel is usually comprised of an aggregation of multiple machine learning models on the
 * translations engine level. The engine has already handled this abstraction.
 *
 * @property language The specified language the language model set can process.
 * @property status The download status of the language models, which can be not downloaded, download processing, or
 *   downloaded.
...
data class LanguageModel(
    ...
    companion object {
         ...
         *
         * A pivot language is used when there is no direct model to translate between two given translation pairs. For
         * example, de -> es is not a model in the translations engine. To accomplish this translation, the models de ->
         * en and en -> es will be used. English is an intermediary or pivot for this translation.
         */
        const val PIVOT_LANGUAGE_CODE = "en"

        /**
         * Convenience method to determine if the [PIVOT_LANGUAGE_CODE] will need to begin syncing on a given operation
         *
         ...
         * @return Whether the [PIVOT_LANGUAGE_CODE] language model should change state as well based on the given
         *   information.
         */
        fun shouldPivotSync(
            appLanguage: String?,
         ...
         * @return Will return true when the pivot language is listed as downloaded by the engine or otherwise not
         *   needed. Will return false when downloaded and needed.
         */
        ...
        /**
         * Convenience method to make the updated language model state based on an [ModelManagementOptions] operation.
         *
         * @param appLanguage The BCP-47 language code for the current app language.
            ...
            when (options.operationLevel) {
                OperationLevel.LANGUAGE -> {
                    ...
                    var updatedModels = currentLanguageModels?.map { model ->
                        if (
                            (model.language?.code == options.languageToManage) &&
        ...
        /**
         * Helper method to determine if changing from one proposed status to another is possible or if it will result
         * in no operation on the engine side.
         *
         * @param currentStatus The current status of the language model.
         * @param newStatus The proposed status the state should move to.
         * @return Will return true if the status change will result in a change. Will return false if the engine is
         *   expected to have a no op operation.
         */
        @VisibleForTesting
        fun checkIfOperable(currentStatus: ModelState, newStatus: ModelState) =
            when (currentStatus) {
