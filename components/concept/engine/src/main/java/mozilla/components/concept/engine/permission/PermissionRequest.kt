...
/**
 * Represents a permission request, used when engines need access to protected resources. Every request must be handled
 * by either calling [grant] or [reject].
 */
interface PermissionRequest {
    ...
    /**
     * Notify the underlying engine that the UI for this permission request has been shown to the user. Implementations
     * that surface a prompt are expected to call this once the prompt is visible, so that the engine can record the
     * prompt display state (e.g. for telemetry).
     */
    @ExperimentalAndroidComponentsApi fun notifyShown() = Unit
}
