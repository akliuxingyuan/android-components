...
@VisibleForTesting internal const val SCROLL_DIRECTIONS_NONE = 0
 ...
 */
@VisibleForTesting internal const val SCROLL_DIRECTIONS_TOP = 1 shl 0
 ...
 */
@VisibleForTesting internal const val SCROLL_DIRECTIONS_RIGHT = 1 shl 1
 ...
 */
@VisibleForTesting internal const val SCROLL_DIRECTIONS_BOTTOM = 1 shl 2
 ...
 */
@VisibleForTesting internal const val SCROLL_DIRECTIONS_LEFT = 1 shl 3

/** The website content cannot be overscrolled. */
@VisibleForTesting internal const val OVERSCROLL_DIRECTIONS_NONE = 0
 ...
 */
@VisibleForTesting internal const val OVERSCROLL_DIRECTIONS_HORIZONTAL = 1 shl 0
 ...
 */
@VisibleForTesting internal const val OVERSCROLL_DIRECTIONS_VERTICAL = 1 shl 1
 ...
 *
 * @param inputResult Indicates who will use the current [MotionEvent]. Possible values: [[INPUT_HANDLING_UNKNOWN],
 *   [INPUT_UNHANDLED], [INPUT_HANDLED], [INPUT_HANDLED_CONTENT]].
 * @param scrollDirections Bitwise ORed value of the directions the page can be scrolled to. This is the same as
 *   GeckoView's [org.mozilla.geckoview.PanZoomController.ScrollableDirections].
 * @param overscrollDirections Bitwise ORed value of the directions the page can be overscrolled to. This is the same as
 *   GeckoView's [org.mozilla.geckoview.PanZoomController.OverscrollDirections].
 */
...
private constructor(
    ...
    /**
     * The website handled the last [MotionEvent] through it's own touch listeners and consumed it without the
     * [EngineView] panning or zooming the website
     */
     ...
     *
     * This might happen on a website without touch listeners that is not bigger than the screen or when the content has
     * no scrollable element.
     */
    ...
    fun canScrollToLeft(): Boolean = inputResult == INPUT_HANDLED && scrollDirections and SCROLL_DIRECTIONS_LEFT != 0

    /** Whether the height of the webpage exceeds the display and the webpage can be scrolled to top. */
    fun canScrollToTop(): Boolean = inputResult == INPUT_HANDLED && scrollDirections and SCROLL_DIRECTIONS_TOP != 0

    /** Whether the width of the webpage exceeds the display and the webpage can be scrolled to right. */
    fun canScrollToRight(): Boolean = inputResult == INPUT_HANDLED && scrollDirections and SCROLL_DIRECTIONS_RIGHT != 0
    ...
    fun canScrollToBottom(): Boolean =
        inputResult == INPUT_HANDLED && scrollDirections and SCROLL_DIRECTIONS_BOTTOM != 0

    /**
     * Whether the webpage can be overscrolled to the left.
     *
     * @return `true` if the page is already scrolled to the left most part and the touch event is not handled by the
     *   webpage.
     */
     ...
     *
     * @return `true` if the page is already scrolled to the top most part and the touch event is not handled by the
     *   webpage.
     */
     ...
     *
     * @return `true` if the page is already scrolled to the right most part and the touch event is not handled by the
     *   webpage.
     */
     ...
     *
     * @return `true` if the page is already scrolled to the bottom most part and the touch event is not handled by the
     *   webpage.
     */
    ...
    @VisibleForTesting
    internal fun getInputResultHandledDescription() =
        when (inputResult) {
    ...
    companion object {
         ...
         *
         * @param verticalOverscrollInitiallyEnabled optional parameter for enabling pull to refresh in the cases in
         *   which this class can be used before valid values being set and it helps more to have overscroll vertically
         *   allowed and then stop depending on the values with which this class is updated rather than start with a
         *   disabled overscroll functionality for the current gesture.
         */
        fun newInstance(verticalOverscrollInitiallyEnabled: Boolean = false) =
            InputResultDetail(
                overscrollDirections =
                    if (verticalOverscrollInitiallyEnabled) {
