...
interface WebExtensionRuntime {
     ...
     * @param url the url pointing to either a resources path for locating the extension within the APK file (e.g.
     *   resource://android/assets/extensions/my_web_ext) or to a local (e.g.
     *   resource://android/assets/extensions/my_web_ext.xpi) XPI file. An error is thrown if a non-resource URL is
     *   passed.
     * @param onSuccess (optional) callback invoked if the extension was installed successfully, providing access to the
     *   [WebExtension] object for bi-directional messaging.
     * @param onError (optional) callback invoked if there was an error installing the extension. This callback is
     *   invoked with an [UnsupportedOperationException] in case the engine doesn't have web extension support.
     */
     ...
     * @param onSuccess (optional) callback invoked if the extension was updated successfully, providing access to the
     *   [WebExtension] object for bi-directional messaging, if null is provided that means that the [WebExtension]
     *   hasn't been change since the last update.
     * @param onError (optional) callback invoked if there was an error updating the extension. This callback is invoked
     *   with an [UnsupportedOperationException] in case the engine doesn't have web extension support.
     */
     ...
     * @param onError (optional) callback invoked if there was an error uninstalling the extension. This callback is
     *   invoked with an [UnsupportedOperationException] in case the engine doesn't have web extension support.
     */
     ...
     * @param onError (optional) callback invoked if there was an error querying the installed extensions. This callback
     *   is invoked with an [UnsupportedOperationException] in case the engine doesn't have web extension support.
     */
    ...
    /**
     * Enables the provided [WebExtension]. If the extension is already enabled the [onSuccess] callback will be
     * invoked, but this method has no effect on the extension.
     *
     ...
     * @param onError (optional) callback invoked if there was an error enabling the extensions. This callback is
     *   invoked with an [UnsupportedOperationException] in case the engine doesn't have web extension support.
     */
     ...
     * @param onError (optional) callback invoked if there was an error adding permissions to the [WebExtension]. This
     *   callback is invoked with an [UnsupportedOperationException] in case the engine doesn't have web extension
     *   support.
     */
     ...
     * @param onError (optional) callback invoked if there was an error removing permissions from the [WebExtension].
     *   This callback is invoked with an [UnsupportedOperationException] in case the engine doesn't have web extension
     *   support.
     */
    ...
    /**
     * Disables the provided [WebExtension]. If the extension is already disabled the [onSuccess] callback will be
     * invoked, but this method has no effect on the extension.
     *
     * @param extension the extension to disable.
     * @param source [EnableSource] to indicate why the extension is disabled.
     * @param onSuccess (optional) callback invoked with the enabled [WebExtension]
     * @param onError (optional) callback invoked if there was an error disabling the installed extensions. This
     *   callback is invoked with an [UnsupportedOperationException] in case the engine doesn't have web extension
     *   support.
     */
     ...
     */
    fun registerWebExtensionDelegate(webExtensionDelegate: WebExtensionDelegate): Unit =
     ...
     * @param onError (optional) callback invoked if there was an error setting private browsing preference the
     *   installed extensions. This callback is invoked with an [UnsupportedOperationException] in case the engine
     *   doesn't have web extension support.
     */
