...
interface TrackingProtectionExceptionStorage {
     ...
     *
     * @param onResult A callback to inform that the domains in the exception list has been fetched, it provides a list
     *   of all the domains that are on the exception list, if there are none domains in the exception list, an empty
     ...
     * @param persistInPrivateMode Indicates if the exception should be persistent in private mode defaults to false.
     */
     ...
     * @param onResult A callback to inform if the given [session] is in the exception list, true if it is in, otherwise
     *   false.
     */
     ...
     *
     * @param activeSessions A list of all active sessions (including CustomTab sessions) to be notified.
