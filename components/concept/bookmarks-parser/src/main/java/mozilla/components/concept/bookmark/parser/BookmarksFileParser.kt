...
sealed class BookmarksParserError(
    ...
    class UnsupportedContentType :
        BookmarksParserError(message = "Expected an HTML file with Netscape doctype but found something else")

    /**
     * Error encountered when there is an issue with the format of the file. This would typically apply when the content
     * was detected to be HTML and Netscape doctype, but an expected value was missing - e.g a bookmark without a link.
     */
    class InvalidFormatError(override val message: String) : BookmarksParserError(message = message)

    /**
     * Error returned when other parsing-related error - I/O errors, or problems with the parsing code, etc., happen.
     */
...
/** An abstract definition of a component that parses bookmark files into a tree of [InsertableBookmarkTreeNode]s. */
fun interface BookmarksFileParser {

    /**
     * Parses the content in an [inputStream] and returns a [BookmarksParseResult] that contains the parsed tree, and
     * some meta-information about the parsing.
     *
     * @param inputStream [InputStream] representing the Netscape bookmark file content
     * @return A [Result] containing the root BookmarksParseResult] that contains the parsed tree, and some
     *   meta-information about the parsing or a failure if the file could not be parsed.
     */
    ...
    companion object {
        ...
        /**
         * Returns a [BookmarksFileParser] that always succeeds, returning [folder] if provided or a default tree
         * otherwise.
         */
        fun fakeSuccess(folder: InsertableBookmarkTreeNode.Folder?): BookmarksFileParser = FakeSuccessParser(folder)
...
private class FakeSuccessParser(val returnedFolder: InsertableBookmarkTreeNode.Folder?) : BookmarksFileParser {
