 ...
 *
 * If you want to add a profiler marker to mark a point in time (without a duration) you can directly use
 * `engine.profiler?.addMarker("marker name")`. Or if you want to provide more information, you can use
 * `engine.profiler?.addMarker("marker name", "extra information")`.
 *
 * If you want to add a profiler marker with a duration (with start and end time) you can use it like this, it will
 * automatically get the end time inside the addMarker:
 * ```
 ...
 * ```
 *
 * [Profiler.isProfilerActive] method is handy when you want to get more information to add inside the marker, but you
 * think it's going to be computationally heavy (and useless) when profiler is not running:
 * ```
 *     val startTime = engine.profiler?.getProfilerTime()
 *     ...some code you want to measure...
...
interface Profiler {
    /**
     * Returns true if profiler is active and it's allowed the add markers. It's useful when it's computationally heavy
     * to get startTime or the additional text for the marker. That code can be wrapped with isProfilerActive if check
     * to reduce the overhead of it.
     *
     ...
     *
     * <code> val startTime = engine.profiler?.getProfilerTime() ...some code you want to measure...
     * engine.profiler?.addMarker("name", startTime) </code>
     *
    ...
    /**
     * Add a profiler marker to Gecko Profiler with the given arguments. It can be used for either adding a
     * point-in-time marker or a duration marker. No-op if profiler is not active.
     *
    ...
    /**
     * Add a profiler marker to Gecko Profiler with the given arguments. End time will be added automatically with the
     * current profiler time when the function is called. No-op if profiler is not active. This is an overload of
     * [Profiler.addMarker] for convenience.
     *
    ...
    /**
     * Add a profiler marker to Gecko Profiler with the given arguments. Time will be added automatically with the
     * current profiler time when the function is called. No-op if profiler is not active. This is an overload of
     * [Profiler.addMarker] for convenience.
     *
     * @param markerName Name of the event as a string.
    ...
    /**
     * Start the Gecko profiler with the given settings. This is used by embedders which want to control the profiler
     * from the embedding app. This allows them to provide an easier access point to profiling, as an alternative to the
     * traditional way of using a desktop Firefox instance connected via USB + adb.
     *
     * @param aFilters The list of threads to profile, as an array of string of thread names filters. Each filter is
     *   used as a case-insensitive substring match against the actual thread names.
     ...
     *
     * @return GeckoResult for the captured profile. The profile is returned as a byte[] buffer containing a
     *   gzip-compressed payload (with gzip header) of the profile JSON.
     */
