...
sealed class PasswordsParserError(
    ...
    /**
     * The file is missing a required column (origin, username, or password) or could not be recognized as a passwords
     * file at all.
     */
    class FileFormatError(message: String) : PasswordsParserError(message)

    /**
     * Two distinct columns in the header map to the same logical field — for example, a file containing both `url` and
     * `login_uri`. The correct value cannot be chosen unambiguously.
     */
 ...
 * @property skippedRowCount The number of rows that were skipped because a required value (origin, username, or
 *   password) was empty.
 */
...
/** An abstract definition of a component that parses password files into a list of [LoginEntry] records. */
fun interface PasswordsFileParser {
     ...
     * @return A [Result] containing the parsed [PasswordsParseResult] on success, or a [PasswordsParserError] on
     *   failure.
     */
    ...
    companion object {
        ...
        fun fakeSuccess(logins: List<LoginEntry> = emptyList()): PasswordsFileParser = PasswordsFileParser {
