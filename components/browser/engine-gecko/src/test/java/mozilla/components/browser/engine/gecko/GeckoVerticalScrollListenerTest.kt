...
class GeckoVerticalScrollListenerTest {
    ...
    @Test
    fun `WHEN there is no scroll data THEN return 0 for scroll and delta`() =
    ...
    @Test
    fun `GIVEN multiple scroll and zoom updates in a GeckoSession WHEN observing them THEN get the scroll values and scroll deltas only if not zooming`() =
        runTest(testScheduler) {
            val scrollListener = GeckoVerticalScrollListener(testDispatcher)
    ...
    @Test
    fun `GIVEN a new GeckoSession WHEN observing it for scroll details THEN resets reset current data and start observing the new session`() =
        runTest(testScheduler) {
            val scrollListener = GeckoVerticalScrollListener(testDispatcher)
            val positions = mutableListOf<Float>()
            scrollListener.scrollYPosition.onEach { positions.add(it) }.launchIn(backgroundScope)
    ...
    @Test
    fun `GIVEN already reporting scroll updates for a GeckoSession WHEN called to release the session THEN stop reporting scroll updates`() =
        runTest(testScheduler) {
            val scrollListener = GeckoVerticalScrollListener(testDispatcher)
            val positions = mutableListOf<Float>()
            scrollListener.scrollYPosition.onEach { positions.add(it) }.launchIn(backgroundScope)
