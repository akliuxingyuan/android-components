...
class GeckoMediaDelegateTest {
    ...
    fun `WHEN onRecordingStatusChanged is called THEN notify onRecordingStateChanged`() {
        ...
        val geckoRecordingDevice =
            createGeckoRecordingDevice(
    ...
    fun `GIVEN a GeckoRecordingDevice status WHEN calling toStatus THEN covert to the RecordingDevice status`() {
        val geckoRecordingDevice = createGeckoRecordingDevice(status = GeckoRecordingDevice.Status.RECORDING)
        val geckoInactiveDevice = createGeckoRecordingDevice(status = GeckoRecordingDevice.Status.INACTIVE)
    ...
    fun `GIVEN an invalid GeckoRecordingDevice status WHEN calling toStatus THEN throw an exception`() {
        val geckoInvalidDevice = createGeckoRecordingDevice(status = 12)
        try {
    ...
    fun `GIVEN a GeckoRecordingDevice type WHEN calling toType THEN covert to the RecordingDevice type`() {
        val geckoCameraDevice = createGeckoRecordingDevice(type = GeckoRecordingDevice.Type.CAMERA)
        val geckoMicDevice = createGeckoRecordingDevice(type = GeckoRecordingDevice.Type.MICROPHONE)
    ...
    fun `GIVEN an invalid GeckoRecordingDevice type WHEN calling toType THEN throw an exception`() {
        val geckoInvalidDevice = createGeckoRecordingDevice(type = 12)
        try {
