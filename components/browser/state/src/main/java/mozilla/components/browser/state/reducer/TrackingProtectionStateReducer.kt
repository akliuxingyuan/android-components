...
internal object TrackingProtectionStateReducer {
    /**
     * [TrackingProtectionAction] Reducer function for modifying a specific [TrackingProtectionState] of a
     * [SessionState].
     */
    fun reduce(state: BrowserState, action: TrackingProtectionAction): BrowserState =
        when (action) {
            is TrackingProtectionAction.ToggleAction ->
                state.copyWithTrackingProtectionState(action.tabId) {
            ...
            is TrackingProtectionAction.TrackerLoadedAction ->
                state.copyWithTrackingProtectionState(action.tabId) {
                    it.copy(loadedTrackers = it.loadedTrackers + action.tracker)
                }
            is TrackingProtectionAction.ClearTrackersAction ->
                state.copyWithTrackingProtectionState(action.tabId) {
                    it.copy(loadedTrackers = emptyList(), blockedTrackers = emptyList())
                }
            is TrackingProtectionAction.ToggleExclusionListAction ->
                state.copyWithTrackingProtectionState(action.tabId) {
