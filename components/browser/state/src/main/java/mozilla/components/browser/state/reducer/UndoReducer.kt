...
internal object UndoReducer {
    /** [UndoAction] Reducer function for modifying the [UndoHistoryState] used to undo the removal of tabs. */
    fun reduce(state: BrowserState, action: UndoAction): BrowserState {
        return when (action) {
            is UndoAction.AddRecoverableTabs -> {
                ...
                state.copy(undoHistory = UndoHistoryState(action.tag, action.tabs, action.selectedTabId))
            }
            ...
            is UndoAction.RestoreRecoverableTabs -> {
                ...
                state.copy(undoHistory = UndoHistoryState())
            }
