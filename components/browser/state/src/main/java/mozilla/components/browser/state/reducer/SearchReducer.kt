...
internal object SearchReducer {
    ...
    fun reduce(state: BrowserState, action: SearchAction): BrowserState {
        return when (action) {
            ...
            is SearchAction.UpdateDisabledSearchEngineIdsAction -> state.updateDisabledSearchEngineIds(action)
            is SearchAction.RestoreHiddenSearchEnginesAction -> state.restoreHiddenSearchEngines()
        }
    }
}

private fun BrowserState.setApplicationEngines(action: SearchAction.ApplicationSearchEnginesLoaded): BrowserState {
    return copy(search = search.copy(applicationSearchEngines = action.applicationSearchEngines))
}

private fun BrowserState.setSearchEngines(action: SearchAction.SetSearchEnginesAction): BrowserState {
    return copy(
        search =
            search.copy(
...
) = copy(search = search.copy(isNewSearchConfigurationAvailable = action.isNewSearchConfigurationAvailable))

private fun BrowserState.setRegion(action: SearchAction.SetRegionAction): BrowserState {
    return copy(search = search.copy(region = action.regionState))
}

private fun BrowserState.updateCustomSearchEngine(action: SearchAction.UpdateCustomSearchEngineAction): BrowserState {
    ...
    return copy(search = search.copy(customSearchEngines = updatedSearchEngines))
}

private fun BrowserState.removeSearchEngine(action: SearchAction.RemoveCustomSearchEngineAction): BrowserState {
    return copy(
        search = search.copy(customSearchEngines = search.customSearchEngines.filter { it.id != action.searchEngineId })
    )
}

private fun BrowserState.selectSearchEngine(action: SearchAction.SelectSearchEngineAction): BrowserState {
...
private fun BrowserState.selectPrivateSearchEngine(action: SearchAction.SelectPrivateSearchEngineAction): BrowserState {
    return copy(
        search =
            search.copy(
...
private fun BrowserState.maybeShowSearchEngine(action: SearchAction.ShowSearchEngineAction): BrowserState {
...
private fun BrowserState.showSearchEngine(searchEngine: SearchEngine): BrowserState {
    return copy(
        search =
            search.copy(
                hiddenSearchEngines = search.hiddenSearchEngines - searchEngine,
                regionSearchEngines =
                    (search.regionSearchEngines + searchEngine).sortedBy {
...
private fun BrowserState.hideSearchEngine(action: SearchAction.HideSearchEngineAction): BrowserState {
...
private fun BrowserState.addAdditionalSearchEngine(action: SearchAction.AddAdditionalSearchEngineAction): BrowserState {
    val searchEngine =
        search.additionalAvailableSearchEngines.find { searchEngine ->
...
private fun BrowserState.removeAdditionalSearchEngine(
    ...
    val searchEngine =
        search.additionalSearchEngines.find { searchEngine ->
            searchEngine.id == action.searchEngineId
        }
...
private fun BrowserState.updateDisabledSearchEngineIds(
    ...
    val updatedDisabledSearchEngineShortcutIds =
        if (action.isEnabled) {
    ...
    return copy(search = search.copy(disabledSearchEngineIds = updatedDisabledSearchEngineShortcutIds))
}
