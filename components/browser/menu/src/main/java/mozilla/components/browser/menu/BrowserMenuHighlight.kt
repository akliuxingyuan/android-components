...
/**
 * Describes how to display a [mozilla.components.browser.menu.item.BrowserMenuHighlightableItem] when it is
 * highlighted.
 */
sealed class BrowserMenuHighlight {
    ...
    /**
     * Displays a notification dot. Used for highlighting new features to the user, such as what's new or a recommended
     * feature.
     *
     ...
     * @property canPropagate Indicate whether other components should consider this highlight when displaying their own
     *   highlight.
     */
    data class LowPriority(
        ...
        override fun asEffect(context: Context) = LowPriorityHighlightEffect(notificationTint = notificationTint)
    }

    /**
     * Changes the background of the menu item. Used for errors that require user attention, like sync errors.
     *
     * @property backgroundTint Tint for the menu item background color. Also used to highlight the menu button.
     * @property label Label to override the normal label of the menu item.
    ...
    data class HighPriority(
        ...
        override fun asEffect(context: Context) = HighPriorityHighlightEffect(backgroundTint = backgroundTint)
    }

    /**
     * Described how to display a highlightable menu item when it is highlighted. Replaced by [LowPriority] and
     * [HighPriority] which lets a priority be specified. This class only exists so that
     * [mozilla.components.browser.menu.item.BrowserMenuHighlightableItem.Highlight] can subclass it.
     *
     * @property canPropagate Indicate whether other components should consider this highlight when displaying their own
     *   highlight.
     */
    ...
    open class ClassicHighlight(
        ...
        override fun asEffect(context: Context) =
            HighPriorityHighlightEffect(backgroundTint = ContextCompat.getColor(context, colorResource))
    }
}
