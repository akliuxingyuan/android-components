...
/**
 * Provides autocomplete functionality for domains, based on a provided list of assets (see [Domains]) and/or a custom
 * domain list managed by [CustomDomains].
 */
...
class DomainAutoCompleteProvider {
     ...
     *
     * @property text the result text starting with the raw search text as passed to [autocomplete] followed by the
     *   completion text (e.g. moz => mozilla.org)
     * @property url the complete url (containing the protocol) as provided when the domain was saved. (e.g.
     *   https://mozilla.org)
     * @property source the source identifier of the autocomplete source
     * @property size total number of available autocomplete domains in this source
     */
    ...
    @Volatile internal var customDomains = emptyList<Domain>()

    @Volatile internal var shippedDomains = emptyList<Domain>()
     ...
     * @return the result of auto-completion. If no match is found an empty result object is returned.
     */
     ...
     * @param useShippedDomains true (default) if the domains provided by this module should be used, otherwise false.
     * @param useCustomDomains true if the custom domains provided by [CustomDomains] should be used, otherwise false
     *   (default).
     * @param loadDomainsFromDisk true (default) if domains should be loaded, otherwise false. This parameter is for
