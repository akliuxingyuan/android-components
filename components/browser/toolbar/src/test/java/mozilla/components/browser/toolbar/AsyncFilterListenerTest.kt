...
class AsyncFilterListenerTest {
    ...
    @Test
    fun `filter listener triggers filter execution`() =
    ...
    @Test
    fun `filter delegate passes results to autocomplete view`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `delegate discards stale results`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `delegate discards results if scope was cancelled`() =
        runTest(testDispatcher) {
            ...
            var called = false
            preservedDelegate?.applyAutocompleteResult(AutocompleteResult("first", "text", "url", "source", 1)) {
                called = true
            }
    ...
    @Test
    fun `delegate passes through non-stale lack of results`() =
        runTest(testDispatcher) {
            val filter: suspend (String, AutocompleteDelegate) -> Unit = { query, delegate ->
