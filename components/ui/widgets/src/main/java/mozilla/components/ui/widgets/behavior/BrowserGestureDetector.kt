...
/**
 * Wraps exceptions that are caught by [BrowserGestureDetector]. Instances of this class are submitted via
 * [CrashReporting]. This wrapping helps easily identify exceptions related to [BrowserGestureDetector].
 */
...
internal class BrowserGestureDetector(
     ...
     *
     * Applications should pass a complete and consistent event stream to this method. A complete and consistent event
     * stream involves all MotionEvents from the initial ACTION_DOWN to the final ACTION_UP or ACTION_CANCEL.
     *
     ...
     *
     * Provide implementation for the events you are interested in. The others will be no-op.
     */
    internal class GesturesListener(
        /**
         * Responds to scroll events for a gesture in progress. The distance in x and y is also supplied for
         * convenience.
         */
        val onScroll: ((distanceX: Float, distanceY: Float) -> Unit)? = { _, _ -> run {} },
        /**
         * Responds to an in progress scroll occuring more on the vertical axis. The scroll distance is also supplied
         * for convenience.
         */
        val onVerticalScroll: ((distance: Float) -> Unit)? = {},
        /**
         * Responds to an in progress scroll occurring more on the horizontal axis. The scroll distance is also supplied
         * for convenience.
         */
        val onHorizontalScroll: ((distance: Float) -> Unit)? = {},
        /** Responds to the the beginning of a new scale gesture. Reported by new pointers going down. */
        val onScaleBegin: ((scaleFactor: Float) -> Unit)? = {},
        /**
         * Responds to scaling events for a gesture in progress. The scaling factor is also supplied for convenience.
