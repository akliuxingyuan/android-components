...
class CrashMiddlewareTest {
    ...
    @Test
    fun `GIVEN crash reporting is initializing WHEN user has chosen to always send crashes THEN skip checking whether we've recently deferred reporting`() =
        runTest {
    ...
    @Test
    fun `GIVEN crash reporting is initialized WHEN user has not chosen to always send crashes THEN check if the user has deferred sending crash reports`() =
        runTest {
            val cache: CrashReportCache = mock()
            val middleware = CrashMiddleware(cache, mock(), mock(), this)
            val middlewareContext: Pair<() -> CrashState, (CrashAction) -> Unit> = Pair(mock(), mock())
    ...
    @Test
    fun `GIVEN a crash reporter that is checking if it should defer WHEN we get a result from the cache THEN try restoring the result`() =
        runTest {
            val cache: CrashReportCache = mock()
    ...
    @Test
    fun `GIVEN a crash reporter that is checking if it should defer WHEN we get a null from the cache THEN check for crashes`() =
        runTest {
            val cache: CrashReportCache = mock()
            val middleware = CrashMiddleware(cache, mock(), { 999_999 }, this)
            val middlewareContext: Pair<() -> CrashState, (CrashAction) -> Unit> = Pair(mock(), mock())
    ...
    @Test
    fun `GIVEN a crash reporter is ready WHEN handling RestoreDefer THEN clear cache and check for crashes`() =
        runTest {
            val cache: CrashReportCache = mock()
            val middleware = CrashMiddleware(cache, mock(), mock(), this)
            val middlewareContext: Pair<() -> CrashState, (CrashAction) -> Unit> = Pair(mock(), mock())
    ...
    @Test
    fun `GIVEN a crash reporter that has unsent crashes and no cutoff date WHEN handling the check for crashes THEN set cutoff date and dispatch FinishCheckingForCrashes with hasUnsentCrashes = true`() =
        runTest {
    ...
    @Test
    fun `GIVEN a crash reporter that has unsent crashes and a cutoff date WHEN handling the check for crashes THEN dispatch FinishCheckingForCrashes with hasUnsentCrashes = true`() =
        runTest {
            val crashReporter: CrashReporter = mock()
            val cache: CrashReportCache = mock()
            val middleware = CrashMiddleware(cache, crashReporter, { 777L }, this)
    ...
    @Test
    fun `GIVEN a crash reporter that has unsent crashes WHEN handling the check for crashes THEN dispatch FinishCheckingForCrashes with hasUnsentCrashes = true`() =
        runTest {
            val crashReporter: CrashReporter = mock()
    ...
    @Test
    fun `GIVEN a crash reporter that has no unsent crashes WHEN handling the check for crashes THEN dispatch FinishCheckingForCrashes with hasUnsentCrashes = false`() =
        runTest {
            val crashReporter: CrashReporter = mock()
            val middleware = CrashMiddleware(mock(), crashReporter, { 777L }, this)
            val middlewareContext: Pair<() -> CrashState, (CrashAction) -> Unit> = Pair(mock(), mock())
    ...
    @Test
    fun `GIVEN a crash reporter that has no unsent crashes and crashes aren't being pulled WHEN handling FinishCheckingForCrashes THEN no crashes are sent`() =
        runTest {
            val cache: CrashReportCache = mock()
            `when`(cache.getReportOption()).thenReturn(CrashReportOption.Auto)
    ...
    @Test
    fun `GIVEN FinishCheckingForCrashes action WHEN hasUnsentCrashes is true AND user has chosen to always send crashes THEN send unsent crash reports`() =
        runTest {
            val cache: CrashReportCache = mock()
            val crashReporter: CrashReporter = mock()
            val middleware = CrashMiddleware(cache, crashReporter, { 777L }, this)
            ...
            `when`(cache.getReportOption()).thenReturn(CrashReportOption.Auto)
            `when`(crashReporter.unsentCrashReportsSince(777L))
                .thenReturn(listOf(mock<Crash.UncaughtExceptionCrash>(), mock<Crash.UncaughtExceptionCrash>()))
            val (_, dispatcher) = middlewareContext
    ...
    @Test
    fun `GIVEN FinishCheckingForCrashes action WHEN hasUnsentCrashes is true AND user has not chosen to always send crashes THEN dispatch ShowPrompt`() =
        runTest {
            val cache: CrashReportCache = mock()
            val crashReporter: CrashReporter = mock()
            val middleware = CrashMiddleware(cache, crashReporter, mock(), this)
    ...
    @Test
    fun `GIVEN ReportTapped WHEN automaticallySendChecked is true and crashIDs is null THEN submit unsent crashes and cache value`() =
        runTest {
            val cache: CrashReportCache = mock()
            val crashReporter: CrashReporter = mock()
            val middleware = CrashMiddleware(cache, crashReporter, { 777L }, this)
    ...
    @Test
    fun `GIVEN ReportTapped WHEN automaticallySendChecked is false and crashIDs is null THEN submit unsent crashes`() =
        runTest {
            val cache: CrashReportCache = mock()
            val crashReporter: CrashReporter = mock()
            val middleware = CrashMiddleware(cache, crashReporter, { 777L }, this)
            ...
            middleware.invoke(
                mock(),
                mock(),
                CrashAction.ReportTapped(automaticallySendChecked = false, crashIDs = listOf()),
            )
            testScheduler.advanceUntilIdle()
    ...
    @Test
    fun `GIVEN ReportTapped WHEN automaticallySendChecked is true and crashIDs is not null THEN ignore automaticallySendChecked`() =
        runTest {
            val cache: CrashReportCache = mock()
            val crashReporter: CrashReporter = mock()
            val middleware = spy(CrashMiddleware(cache, crashReporter, { 0L }, this))
            ...
            middleware.invoke(
                mock(),
                mock(),
                CrashAction.ReportTapped(automaticallySendChecked = true, crashIDs = crashIDs),
            )
            testScheduler.advanceUntilIdle()
    ...
    @Test
    fun `GIVEN ReportTapped WHEN automaticallySendChecked is false and some crashIDs THEN submit those crashes and set deferral`() =
        runTest {
            val cache: CrashReportCache = mock()
            val crashReporter: CrashReporter = mock()
            ...
            `when`(crashReporter.findCrashReports(crashIDs))
                .thenReturn(listOf(mock<Crash.UncaughtExceptionCrash>(), mock<Crash.UncaughtExceptionCrash>()))
            middleware.invoke(
                mock(),
                mock(),
                CrashAction.ReportTapped(automaticallySendChecked = false, crashIDs = crashIDs.toList()),
            )
            testScheduler.advanceUntilIdle()
    ...
    @Test
    fun `GIVEN checking for crash pull deferral WHEN general deferral period is longer than crash pull deferral THEN crash pulls will respect the general period`() =
        runTest {
    ...
    @Test
    fun `GIVEN checking for crash pull deferral WHEN general deferral period is shorter than crash pull deferral THEN crash pulls will respect the crash pull period`() =
        runTest {
            var capture: CrashAction? = null
            val cache: CrashReportCache = mock()
            val crashReporter: CrashReporter = mock()
    ...
    @Test
    fun `GIVEN checking for crash pull deferral WHEN neither deferral period is set THEN crash pulls check for crashes`() =
        runTest {
            var capture: CrashAction? = null
            val cache: CrashReportCache = mock()
            val crashReporter: CrashReporter = mock()
    ...
    @Test
    fun `WHEN crash pulls are automatically submitted THEN the deferral period is set to avoid repeated submissions`() =
        runTest {
            val cache: CrashReportCache = mock()
            val crashReporter: CrashReporter = mock()
            ...
            val middlewareContext: Pair<() -> CrashState, (CrashAction) -> Unit> = Pair(mock(), mock())
            `when`(cache.getReportOption()).thenReturn(CrashReportOption.Auto)
            `when`(crashReporter.findCrashReports(arrayOf("1")))
                .thenReturn(listOf(mock<Crash.UncaughtExceptionCrash>()))

            val (_, dispatcher) = middlewareContext
            middleware.invoke(
                middlewareContext,
                mock(),
                CrashAction.FinishCheckingForCrashes(hasUnsentCrashes = false, crashIds = listOf("1")),
            )
            testScheduler.advanceUntilIdle()
