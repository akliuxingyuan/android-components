...
internal val stateMachine: Map<State, StateMap> =
    mapOf(
        State.EXPECT_OPERAND to
            StateMap(
                mapOf(
                    Token.Type.LITERAL to NextState(State.EXPECT_BIN_OP),
                    Token.Type.IDENTIFIER to NextState(State.IDENTIFIER),
                    Token.Type.UNARY_OP to NextState(),
                    Token.Type.OPEN_PAREN to NextState(State.SUB_EXPRESSION),
                    Token.Type.OPEN_CURL to
                        NextState(
                            State.EXPECT_OBJECT_KEY,
                            ::objectStart,
                        ),
                    Token.Type.DOT to NextState(State.TRAVERSE),
                    Token.Type.OPEN_BRACKET to
                        NextState(
                            State.ARRAY_VALUE,
                            ::arrayStart,
                        ),
        ...
        State.EXPECT_BIN_OP to
            StateMap(
                mapOf(
                    Token.Type.BINARY_OP to NextState(State.EXPECT_OPERAND),
                    Token.Type.PIPE to NextState(State.EXPECT_TRANSFORM),
                    Token.Type.DOT to NextState(State.TRAVERSE),
                    Token.Type.QUESTION to
                        NextState(
                            State.TERNARY_MID,
                            ::ternaryStart,
                        ),
        ...
        State.EXPECT_OBJECT_KEY to
            StateMap(
                mapOf(
                    ...
                    Token.Type.CLOSE_CURL to NextState(State.EXPECT_BIN_OP),
                )
            ),
        State.EXPECT_KEY_VALUE_SEPARATOR to StateMap(mapOf(Token.Type.COLON to NextState(State.OBJECT_VALUE))),
        State.POST_TRANSFORM to
            StateMap(
                mapOf(
                    Token.Type.OPEN_PAREN to NextState(State.ARGUMENT_VALUE),
                    Token.Type.BINARY_OP to NextState(State.EXPECT_OPERAND),
                    Token.Type.DOT to NextState(State.TRAVERSE),
                    Token.Type.OPEN_BRACKET to NextState(State.FILTER),
                    Token.Type.PIPE to NextState(State.EXPECT_TRANSFORM),
                ),
                completable = true,
            ),
        State.POST_TRANSFORM_ARGUMENTS to
            StateMap(
                mapOf(
                    Token.Type.BINARY_OP to NextState(State.EXPECT_OPERAND),
                    Token.Type.DOT to NextState(State.TRAVERSE),
        ...
        State.TRAVERSE to StateMap(mapOf(Token.Type.IDENTIFIER to NextState(State.IDENTIFIER))),
        State.FILTER to
            StateMap(
                ...
                endStates = mapOf(Token.Type.CLOSE_BRACKET to State.IDENTIFIER),
            ),
        State.SUB_EXPRESSION to
            StateMap(
                subHandler = { parser, node ->
                    parser.placeAtCursor(node!!)
                },
                endStates = mapOf(Token.Type.CLOSE_PAREN to State.EXPECT_BIN_OP),
            ),
        State.ARGUMENT_VALUE to
            StateMap(
                subHandler = { parser, node ->
        ...
        State.TERNARY_MID to
            StateMap(
                subHandler = { parser, node ->
                    val cursor = parser.cursor!! as ConditionalExpression
                    cursor.consequent = node
                },
                endStates = mapOf(Token.Type.COLON to State.TERNARY_END),
            ),
        State.TERNARY_END to
            StateMap(
                subHandler = { parser, node ->
                    val cursor = parser.cursor!! as ConditionalExpression
...
private fun objectStart(
    ...
    val node = ObjectLiteral(properties = mutableMapOf())
    parser.placeAtCursor(node)
}
...
private fun ternaryStart(
    ...
    val node = ConditionalExpression(test = parser.tree)
    parser.tree = node
    parser.cursor = node
}
