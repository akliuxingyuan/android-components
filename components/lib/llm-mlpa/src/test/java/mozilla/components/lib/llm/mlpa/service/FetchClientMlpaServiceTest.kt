...
class FetchClientMlpaServiceTest {
    ...
    @Test
    fun `GIVEN a verification request WHEN made THEN headers include json content type`() = runTest {
        val json =
            """
            {
        ...
        val mlpaService = FetchClientMlpaService(client, MlpaConfig.prodProd)
    ...
    fun `GIVEN a successful response WHEN try to verify an integrity token THEN return a constructed Response`() =
        runTest {
            ...
            val mlpaService = FetchClientMlpaService(FakeClient.success(json.asBody), MlpaConfig.prodProd)
    ...
    @Test
    fun `GIVEN a malformed response WHEN we try to verify an integrity THEN return a failure`() = runTest {
        val json =
            """
            {
    ...
    @Test
    fun `GIVEN a network failure WHEN try to verify THEN return a NetworkError`() = runTest {
    ...
    @Test
    fun `GIVEN a failure response WHEN we try to verify an integrity THEN return a failure`() = runTest {
    ...
    @Test
    fun `GIVEN non-streamed successful response WHEN try to chat THEN return a constructed Response`() = runTest {
        val json =
            """
            {
                "choices": [
    ...
    @Test
    fun `GIVEN streamed success response WHEN try to chat THEN return the content of the response`() = runTest {
    ...
    @Test
    fun `GIVEN a malformed streamed response WHEN try to chat THEN return a parse error`() = runTest {
        ...
        val mlpaService = FetchClientMlpaService(FakeClient.success(malformedStreamBody.asBody), MlpaConfig.prodProd)
    ...
    @Test
    fun `GIVEN a malformed error WHEN try to chat THEN return a parse error`() = runTest {
        val malformedStreamBody =
            """
            data: {"errorasdasd": 1}
    ...
    @Test
    fun `GIVEN a budget limit midstream error WHEN try to chat THEN return an RateLimited error`() = runTest {
        val malformedStreamBody =
            """
            data: {"error": 1}
    ...
    @Test
    fun `GIVEN a rate limit midstream error WHEN try to chat THEN return an RateLimited error`() = runTest {
        val malformedStreamBody =
            """
            data: {"error": 2}
    ...
    @Test
    fun `GIVEN a unknown midstream error WHEN try to chat THEN return a server error`() = runTest {
        val malformedStreamBody =
            """
            data: {"error": 18239}
    ...
    @Test
    fun `GIVEN a malformed response WHEN try to chat THEN return a ResponseParseError`() = runTest {
        val json =
            """
            {
                "not_expected": [
    ...
    @Test
    fun `GIVEN a network failure WHEN try to chat THEN return a NetworkError`() = runTest {
        val mlpaService = FetchClientMlpaService(FakeClient.throwing(), MlpaConfig.prodProd)
    ...
    @Test
    fun `GIVEN a malformed 429 body WHEN try to chat THEN return a RateLimitResponseParseError`() = runTest {
        val mlpaService =
    ...
    @Test
    fun `GIVEN a malformed 502 body WHEN try to chat THEN return an UpstreamResponseParseError`() = runTest {
        val mlpaService =
            FetchClientMlpaService(
    ...
    @Test
    fun `GIVEN an error status code WHEN try to chat THEN return the appropriate error`() = runTest {
        data class Case(val statusCode: Int, val expectedError: MlpaError) {
            val headers
                get() =
                    when (expectedError) {
                        RateLimited(8000L),
                        BudgetExceeded(8000L) -> MutableHeaders("Retry-After" to "8000")
                        else -> MutableHeaders()
                    }
