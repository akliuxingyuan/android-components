 ...
 *
 * This uses a combination of a rolling window, cooldown period, sensitivity, and the number of hits or pulses to
 * determine a shake.
 *
 * @param sensitivity The [ShakeSensitivity] of the detection. This determines how much effort is required to trigger a
 *   shake
 * @param detectionWindowNs The time in nanoseconds during which the movements and hits are observed to determine a
 *   shake gesture
 * @param cooldownPeriodNs The time in nanoseconds after a shake is detected. Any movement or shake that happens during
 *   this period is ignored
 * @param minHits The minimum number of hits/pulses/rapid motion (above the threshold defined by [sensitivity]) that we
 *   need to see to determine that a shake has happened.
 */
...
/**
 * This class is used to accumulate rolling shake data - basically, we want to track acceleration magnitudes across a
 * period of time and if we get enough spikes above a threshold within a window we consider it a shake. This state is
 * accumulated until we have reached enough hits to pass the minimum required.
 */
private data class ShakeState(
    ...
    fun process(
        ...
        val magnitude =
            sqrt(sample.xAccel * sample.xAccel + sample.yAccel * sample.yAccel + sample.zAccel * sample.zAccel)
        ...
        val isExpired = detectionWindowStartNs == 0L || timestampNs - detectionWindowStartNs > detectionWindowNs
        val newHits =
            when {
                isExpired -> 1
     ...
     *
     * It currently is a very simple approximation that infers the direction as [ShakeDirection.Forward] or
     * [ShakeDirection.Reverse] based on the largest acceleration recorded.
     *
     * E.g.:
     * - if we record [x = -3, y = 0, z = 1], we determine the direction to be reverse because the maximum recorded
     *   acceleration had a magnitude of 3, but in the negative direction.
     * - if we record [x = -1, y = 5, z = 3], we determine the direction to be forward because the maximum recorded
     *   acceleration had a magnitude of 5, but in the positive direction.
     *
     * There are more complex algorithms, like doing a dot product of previously recorded samples and the new one, but
...
internal inline val Double.g: Float
    get() = this.toFloat() * SensorManager.GRAVITY_EARTH

/** Helper to convert [Int] to a value as a multiple acceleration due to gravity */
internal inline val Int.g: Float
    get() = this * SensorManager.GRAVITY_EARTH
