...
/** Base alpha value for the fading gradient. Not being fully opaque can help smooth out the edges of the gradient. */
...
@VisibleForTesting internal const val END_SCROLL_OFFSET = 1

/**
 * The LTR mark character which will force all other characters following it to be rendered from left to right. See bug
 * 1973915 and related for the context of why we need this.
 */
...
fun HighlightedDomainUrl(
    ...
    val annotatedUrl =
        remember(url, registrableDomainIndexRange, fadedTextStyle, boldedTextStyle) {
...
internal fun computeDomainEndScrollValue(
    ...
    when (highlightRange?.second == text.length) {
        true -> scrollState.maxValue
        else -> {
            val startIndex = (highlightRange?.first ?: 0).coerceIn(0, text.length)

            val endIndex = (highlightRange?.second?.plus(END_SCROLL_OFFSET) ?: 0).coerceIn(startIndex, text.length)
    ...
    AcornTheme {
        Surface {
            HighlightedDomainUrl(
                ...
                fadedTextStyle =
                    LocalTextStyle.current.merge(TextStyle(color = MaterialTheme.colorScheme.onSurfaceVariant)),
                boldedTextStyle = LocalTextStyle.current.merge(TextStyle(color = MaterialTheme.colorScheme.onSurface)),
