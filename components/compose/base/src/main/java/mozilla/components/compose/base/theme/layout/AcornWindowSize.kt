 ...
 */
enum class AcornWindowSize(private val windowWidthMax: Dp) {
    /** Window sizes between 0 and 600. */
    Small(windowWidthMax = 600.dp),

    /** Window sizes between 600 and 840. */
    Medium(windowWidthMax = 840.dp),

    /** Window sizes 840 or larger. */
    Large(windowWidthMax = Dp.Infinity);

    /**
     * Helper function to determine when to make UI differences for layouts in window sizes other than
    ...
    companion object {
         ...
         * @return The [AcornWindowSize] that corresponds to the current window width.
         */
        fun getWindowSize(context: Context): AcornWindowSize =
            getWindowSizeToken(
        ...
        /**
         * Helper function used to determine when the app's total *window* size is at least the size of a tablet. To
         * determine whether the device's *physical* size is at least the size of a tablet, use
         * [Context.isLargeScreenSize] instead.
         *
         * @return true if the app has a window size that is not [AcornWindowSize.Small].
         */
        @Composable
        fun isLargeWindow(): Boolean =
            getWindowSizeToken(
                    configuration = LocalConfiguration.current,
         ...
         * @return The [AcornWindowSize] calculated from the window's screen width. See [AcornWindowSize] for possible
         *   values.
         */
