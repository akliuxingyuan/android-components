...
class WebAppSiteControlsFeatureTest {
    ...
    fun `unregister receiver on pause`() {
        ...
        val feature =
            WebAppSiteControlsFeature(context, BrowserStore(), "session-id", mock(), notificationsDelegate = mock())
    ...
    fun `reload page when reload action is activated`() {
        ...
        val store =
            BrowserStore(
                BrowserState(customTabs = listOf(createCustomTab("https://www.mozilla.org", id = "session-id")))
            )
    ...
    fun `load monochrome icon if defined in manifest`() {
        ...
        val store = BrowserStore(BrowserState(customTabs = listOf(session)))
