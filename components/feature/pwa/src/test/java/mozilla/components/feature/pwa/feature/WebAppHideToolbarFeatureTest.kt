...
class WebAppHideToolbarFeatureTest {
    ...
    fun `does not hide toolbar if origin is not trusted`() =
        runTest(testDispatcher) {
            ...
            val customTabsStore =
                CustomTabsServiceStore(
                    CustomTabsServiceState(tabs = mapOf(token to mockCustomTabState("https://mozilla.org")))
                )
    ...
    fun `onUrlChanged hides toolbar if URL is in origin`() =
        runTest(testDispatcher) {
            ...
            store.dispatch(ContentAction.UpdateUrlAction(customTabId, "https://mozilla.com/example-page"))
            testDispatcher.scheduler.advanceUntilIdle()

            assertFalse(toolbarVisible)

            store.dispatch(ContentAction.UpdateUrlAction(customTabId, "https://firefox.com/out-of-scope"))
            testDispatcher.scheduler.advanceUntilIdle()

            assertTrue(toolbarVisible)

            store.dispatch(ContentAction.UpdateUrlAction(customTabId, "https://mozilla.com/back-in-scope"))
            testDispatcher.scheduler.advanceUntilIdle()

            assertFalse(toolbarVisible)

            store.dispatch(ContentAction.UpdateUrlAction(customTabId, "https://m.mozilla.com/second-origin"))
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    fun `onUrlChanged hides toolbar if URL is in scope`() =
        runTest(testDispatcher) {
            ...
            store.dispatch(ContentAction.UpdateUrlAction(customTabId, "https://mozilla.github.io/my-app/"))
            testDispatcher.scheduler.advanceUntilIdle()
            ...
            store.dispatch(ContentAction.UpdateUrlAction(customTabId, "https://mozilla.github.io/my-app/sub-page"))
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    fun `onUrlChanged hides toolbar if URL is in ambiguous scope`() =
        runTest(testDispatcher) {
            ...
            store.dispatch(ContentAction.UpdateUrlAction(customTabId, "https://mozilla.github.io/prefix/"))
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    fun `onTrustedScopesChange hides toolbar if URL is in origin`() =
        runTest(testDispatcher) {
            ...
            val store = BrowserStore(BrowserState(customTabs = listOf(tab)))
            val customTabsStore =
                CustomTabsServiceStore(CustomTabsServiceState(tabs = mapOf(token to mockCustomTabState())))
            val feature =
                WebAppHideToolbarFeature(
                    store,
                    customTabsStore,
    ...
    private fun mockCustomTabState(vararg origins: String) =
        CustomTabState(
            relationships =
                origins
                    .map { origin ->
    ...
    private fun mockManifest(scope: String) =
        WebAppManifest(
            name = "Mock",
            startUrl = scope,
            scope = scope,
