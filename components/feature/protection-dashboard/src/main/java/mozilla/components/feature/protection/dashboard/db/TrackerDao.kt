...
internal interface TrackerDao {
    ...
    @Upsert suspend fun upsertTotal(entity: TrackerTotalEntity)
    ...
    @Upsert suspend fun upsertByHost(entity: TrackerByHostEntity)
    ...
    @Query("DELETE FROM tracker_totals") suspend fun deleteAllTotals()

    @Query("DELETE FROM tracker_by_host") suspend fun deleteAllByHost()
}
