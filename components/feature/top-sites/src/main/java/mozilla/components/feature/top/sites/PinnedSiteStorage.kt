...
class PinnedSiteStorage(
    ...
    @VisibleForTesting internal var currentTimeMillis: () -> Long = currentTimeMillis

    @VisibleForTesting internal var database: Lazy<TopSiteDatabase> = lazy { TopSiteDatabase.get(context) }
    ...
    suspend fun getPinnedSites(): List<TopSite> =
        withContext(IO) {
     ...
     */
    suspend fun removePinnedSite(site: TopSite) =
        withContext(IO) {
     ...
     */
    suspend fun updatePinnedSite(site: TopSite, title: String, url: String) =
        withContext(IO) {
    ...
    suspend fun getPinnedSitesCount(): Int =
        withContext(IO) {
