...
class UndoMiddlewareTest {
    ...
    fun `Undo scenario - Removing single tab`() =
        runTest(testDispatcher) {
            val store =
                BrowserStore(
                    middleware = listOf(UndoMiddleware(clearAfterMillis = 60000, this, this)),
                    initialState =
                        BrowserState(
                            tabs =
                                listOf(
            ...
            store.dispatch(TabListAction.RemoveTabAction(tabId = "mozilla"))
    ...
    fun `Undo scenario - Removing list of tabs`() =
        runTest(testDispatcher) {
            ...
            store.dispatch(TabListAction.RemoveTabsAction(listOf("mozilla", "pocket")))
    ...
    fun `Undo scenario - Removing all tabs non-recoverable`() =
        runTest(testDispatcher) {
            ...
            store.dispatch(TabListAction.RemoveAllTabsAction(false))
    ...
    fun `Undo History gets cleared after time`() =
        runTest(testDispatcher) {
            val store =
                BrowserStore(
                    middleware = listOf(UndoMiddleware(clearAfterMillis = 60000, waitScope = this, mainScope = this)),
                    initialState =
                        BrowserState(
                            tabs =
                                listOf(
