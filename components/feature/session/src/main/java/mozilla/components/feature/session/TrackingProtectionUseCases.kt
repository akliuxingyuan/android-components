...
class TrackingProtectionUseCases(
    ...
    internal constructor(
        ...
        /**
         * Adds a new tab to the exception list, as a result this tab will not get applied any tracking protection
         * policy.
         *
         * @param tabId The id of the tab that will be added to the exception list.
         * @param persistInPrivateMode Indicates if the exception should be persistent in private mode defaults to
         *   false.
         */
        operator fun invoke(tabId: String, persistInPrivateMode: Boolean = false) {
            val engineSession =
                store.state.findTabOrCustomTabOrSelectedTab(tabId)?.engineState?.engineSession
    ...
    internal constructor(
        ...
        operator fun invoke(exception: TrackingProtectionException) {
            ...
            val tabs =
                (store.state.tabs + store.state.customTabs).filter { tab ->
    ...
    class RemoveAllExceptionsUseCase
    internal constructor(
        private val store: BrowserStore,
        private val engine: Engine,
    ) {
        ...
        operator fun invoke(onRemove: () -> Unit = {}) {
            val engineSessions =
                (store.state.tabs + store.state.customTabs).mapNotNull { tab ->
    ...
    class ContainsExceptionUseCase
    internal constructor(
        private val store: BrowserStore,
        private val engine: Engine,
    ) {
         ...
         * @param onResult A callback to inform if the given tab is on the exception list, true if it is in otherwise
         *   false.
         */
        operator fun invoke(
            tabId: String,
            ...
            val engineSession =
                store.state.findTabOrCustomTabOrSelectedTab(tabId)?.engineState?.engineSession ?: return onResult(false)
    ...
    class FetchExceptionsUseCase internal constructor(private val engine: Engine) {
        /**
         * Fetch all domains that will be ignored for tracking protection.
         *
         * @param onResult A callback to inform that the domains on the exception list has been fetched, it provides a
         *   list of [TrackingProtectionException] that are on the exception list, if there are none domains on the
         *   exception list, an empty list will be provided.
         */
    ...
    class FetchTrackingLogUserCase
    internal constructor(
        private val store: BrowserStore,
        private val engine: Engine,
    ) {
    ...
    class FetchTrackingEventsUseCase internal constructor(private val engine: Engine) {
        /**
         ...
         * @param onSuccess callback informing about the trackers that have been blocked in the given
    ...
    class FetchTotalTrackersBlockedUseCase internal constructor(private val engine: Engine) {
        /**
    ...
    class FetchEarliestTrackingDateUseCase internal constructor(private val engine: Engine) {
        /**
         * Fetch the earliest recorded date in the tracking protection database.
         *
         * @param onSuccess callback informing about the earliest epoch time (rounded to the earliest hour) from which
         *   we have recorded data about blocked trackers.
         * @param onError (optional) callback invoked if fetching the data caused an exception.
         */
        operator fun invoke(
            onSuccess: (Long?) -> Unit,
