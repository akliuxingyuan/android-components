...
class SyncedTabsStorageTest {
    ...
    @Test
    fun `listens to browser store changes, stores state changes, and calls onStoreComplete`() =
    ...
    @Test
    fun `stops listening to browser store changes on stop()`() =
        runTest(testDispatcher) {
            val feature =
                SyncedTabsStorage(
                    accountManager,
    ...
    @Test
    fun `getSyncedTabs matches tabs with FxA devices`() =
        runTest(testDispatcher) {
            val feature =
                spy(
                    SyncedTabsStorage(
            ...
            whenever(tabsStorage.getAll())
                .thenReturn(
                    mapOf(
                        SyncClient("client1") to tabsClient1,
                        SyncClient("client2") to tabsClient2,
                        SyncClient("client-unknown") to
                            listOf(Tab(listOf(TabEntry("Foo", "https://foo.bar", null)), 0, 0, true)),
                    )
                )
    ...
    @Test
    fun `getSyncedTabs returns empty list if syncClients() is null`() =
        runTest(testDispatcher) {
            val feature =
                spy(
                    SyncedTabsStorage(
    ...
    @Test
    fun `tabs are stored when loaded`() =
        runTest(testDispatcher) {
            val store =
                BrowserStore(
                    BrowserState(
    ...
    @Test
    fun `only loaded tabs are stored on load`() =
        runTest(testDispatcher) {
            val store =
                BrowserStore(
                    BrowserState(
    ...
    @Test
    fun `tabs are stored when selected tab changes`() =
        runTest(testDispatcher) {
            val store =
                BrowserStore(
                    BrowserState(
    ...
    @Test
    fun `tabs are stored when lastAccessed is changed for any tab`() =
        runTest(testDispatcher) {
            val store =
                BrowserStore(
                    BrowserState(
