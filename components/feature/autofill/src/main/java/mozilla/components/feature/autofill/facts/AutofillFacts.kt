...
internal fun emitAutofillConfirmationFact(confirmed: Boolean) {
    Fact(
            Component.FEATURE_AUTOFILL,
            if (confirmed) {
                Action.CONFIRM
...
internal fun emitAutofillLock(unlocked: Boolean) {
    Fact(
            Component.FEATURE_AUTOFILL,
            if (unlocked) {
                Action.CONFIRM
...
private fun requestMetadata(
    ...
    val metadata = mutableMapOf<String, Any>(AutofillFacts.Metadata.HAS_MATCHING_LOGINS to hasLogins)
