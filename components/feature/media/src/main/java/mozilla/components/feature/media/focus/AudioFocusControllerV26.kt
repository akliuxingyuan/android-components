...
internal class AudioFocusControllerV26(
    ...
    /**
     * Shape the focus request after the audio-session [type] so other apps react appropriately. The
     * transient/transient-solo usage values are best-fit choices pending validation with Android engineers (the audio
     * session plan's Open Item #4).
     */
    private fun buildRequest(type: MediaSession.AudioSessionType): AudioFocusRequest {
        val (gain, usage, contentType) =
            when (type) {
                MediaSession.AudioSessionType.PLAY_AND_RECORD ->
                    Triple(
                        AudioManager.AUDIOFOCUS_GAIN,
                        AudioAttributes.USAGE_VOICE_COMMUNICATION,
                        AudioAttributes.CONTENT_TYPE_SPEECH,
                    )
                MediaSession.AudioSessionType.TRANSIENT_SOLO ->
                    Triple(
