...
class CameraManagerTest {
    ...
    fun `isLowLightBoostSupported returns false if availableAeModes is null`() {
        whenever(characteristics.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_MODES)).thenReturn(null)
        assertFalse(cameraManager.isLowLightBoostSupported(cameraId))
    }

    @Test
    fun `isLowLightBoostSupported returns false if low light boost mode is not available`() {
        whenever(characteristics.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_MODES))
            .thenReturn(intArrayOf(CameraMetadata.CONTROL_AE_MODE_ON))
        assertFalse(cameraManager.isLowLightBoostSupported(cameraId))
    }
    ...
    fun `isLowLightBoostSupported returns true if low light boost mode is available`() {
        whenever(characteristics.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_MODES))
            .thenReturn(intArrayOf(CameraMetadata.CONTROL_AE_MODE_ON_LOW_LIGHT_BOOST_BRIGHTNESS_PRIORITY))
