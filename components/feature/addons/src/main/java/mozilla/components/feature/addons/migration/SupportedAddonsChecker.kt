 ...
 * @param frequency (Optional) indicates how often checks should be performed, defaults to once per day.
 */
@Suppress("LargeClass")
class DefaultSupportedAddonsChecker(
    ...
    override fun unregisterForChecks() {
        WorkManager.getInstance(applicationContext).cancelUniqueWork(CHECKER_UNIQUE_PERIODIC_WORK_NAME)
    ...
    @VisibleForTesting
    internal fun getWorkerConstrains() = Constraints.Builder().setRequiredNetworkType(NetworkType.CONNECTED).build()
    ...
    companion object {
        ...
        @VisibleForTesting
        internal const val WORK_TAG_PERIODIC = "$IDENTIFIER_PREFIX.DefaultSupportedAddonsChecker.periodicWork"
    }
}
