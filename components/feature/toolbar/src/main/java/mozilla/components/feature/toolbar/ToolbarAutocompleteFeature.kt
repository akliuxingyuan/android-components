 ...
 * @param engine (optional) instance of a browser [Engine] to issue [Engine.speculativeConnect] calls on successful URL
 *   autocompletion.
 * @param shouldAutocomplete (optional) lambda expression that returns true if autocomplete is shown. Otherwise,
...
class ToolbarAutocompleteFeature(
    ...
    @VisibleForTesting internal var autocompleteProviders: SortedSet<AutocompleteProvider> = sortedSetOf()
    ...
    init {
        toolbar.setAutocompleteListener { query, delegate ->
            if (!shouldAutocomplete() || autocompleteProviders.isEmpty() || query.isBlank()) {
                ...
                val result = autocompleteProviders.firstNotNullOfOrNull { it.getAutocompleteSuggestion(query) }
    ...
    /**
     * Update the list of providers used for autocompletion results. Changes will take effect the next time user changes
     * their input.
     *
     * @param providers New list of autocomplete providers. The list can be empty in which case autocompletion will be
     *   disabled until there is at least one provider.
     * @param refreshAutocomplete Whether to immediately update the autocompletion suggestion based on the new
     *   providers.
     */
    @Synchronized
     ...
     *
     * @param provider New [AutocompleteProvider] to add to the current list. If this exact instance already exists it
     ...
     *
     * @param provider [AutocompleteProvider] instance to remove from the current list. If it isn't set already calling
