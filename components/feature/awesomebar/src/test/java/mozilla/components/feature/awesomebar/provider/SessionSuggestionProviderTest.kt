...
class SessionSuggestionProviderTest {
    ...
    @Test
    fun `WHEN input text has trailing space THEN Provider returns Sessions with matching URLs`() = runTest {
        val store = BrowserStore()
    ...
    @Test
    fun `WHEN input text has leading space THEN Provider returns Sessions with matching URLs`() = runTest {
        val store = BrowserStore()
    ...
    @Test
    fun `GIVEN input text has multiple matching words WHEN some match THEN Provider returns an empty list`() = runTest {
        val store = BrowserStore()
    ...
    fun `Provider only returns non-private Sessions`() = runTest {
        ...
        val store = BrowserStore(BrowserState(tabs = listOf(tab, privateTab1, privateTab2)))
    ...
    @Test
    fun `GIVEN a results uri filter WHEN querying tabs THEN return only the results that pass through the filter`() =
        runTest {
            val store =
                BrowserStore(
                    BrowserState(
    ...
    @Test
    fun `GIVEN multiple tabs have the same url WHEN user inputs the same url THEN provider returns a single suggestion for the matching input`() =
        runTest {
            val store = BrowserStore()
