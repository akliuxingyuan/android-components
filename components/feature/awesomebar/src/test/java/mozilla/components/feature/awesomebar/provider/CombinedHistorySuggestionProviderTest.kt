...
class CombinedHistorySuggestionProviderTest {
    ...
    @Test
    fun `GIVEN history items exists WHEN onInputChanged is called with empty text THEN return empty suggestions list`() =
        runTest {
    ...
    @Test
    fun `GIVEN more suggestions asked than metadata items exist WHEN user changes input THEN return a combined list of suggestions`() =
        runTest {
            val storage: HistoryMetadataStorage = mock()
            doReturn(listOf(historyEntry)).`when`(storage).queryHistoryMetadata(eq("moz"), anyInt())
            val history: HistoryStorage = mock()
            doReturn(listOf(SearchResult("id", "http://www.mozilla.com/firefox/", 10)))
                .`when`(history)
                .getSuggestions(eq("moz"), anyInt())
    ...
    @Test
    fun `GIVEN fewer suggestions asked than metadata items exist WHEN user changes input THEN return suggestions only based on metadata items`() =
        runTest {
            val storage: HistoryMetadataStorage = mock()
            doReturn(listOf(historyEntry)).`when`(storage).queryHistoryMetadata(eq("moz"), anyInt())
            val history: HistoryStorage = mock()
    ...
    @Test
    fun `GIVEN only storage history items exist WHEN user changes input THEN return suggestions only based on storage items`() =
        runTest {
            val metadata: HistoryMetadataStorage = mock()
    ...
    @Test
    fun `GIVEN duplicated metadata and storage entries WHEN user changes input THEN return distinct suggestions`() =
        runTest {
            val storage: HistoryMetadataStorage = mock()
            doReturn(listOf(historyEntry)).`when`(storage).queryHistoryMetadata(eq("moz"), anyInt())
            val history: HistoryStorage = mock()
    ...
    @Test
    fun `GIVEN a combined list of suggestions WHEN history results exist THEN urls are deduped and scores are adjusted`() =
        runTest {
            val metadataEntry1 =
                HistoryMetadata(
            ...
            doReturn(listOf(metadataEntry2, metadataEntry1))
                .`when`(metadataStorage)
                .queryHistoryMetadata(eq("moz"), anyInt())
    ...
    @Test
    fun `WHEN provider max number of suggestions is changed THEN the number of return suggestions is updated`() =
        runTest {
            val history: HistoryStorage = mock()
            val metadata: HistoryMetadataStorage = mock()
            doReturn(emptyList<HistoryMetadata>()).`when`(metadata).queryHistoryMetadata(eq("moz"), anyInt())
    ...
    @Test
    fun `WHEN reset provider max number of suggestions THEN the number of return suggestions is reset to default`() =
        runTest {
            val history: HistoryStorage = mock()
            val metadata: HistoryMetadataStorage = mock()
            doReturn(emptyList<HistoryMetadata>()).`when`(metadata).queryHistoryMetadata(eq("moz"), anyInt())
    ...
    fun `GIVEN a results host filter WHEN querying history THEN return only the results that pass through the filter`() =
        runTest {
            ...
            doReturn(
                ...
                .`when`(history)
                .getSuggestions(anyString(), anyInt())
    ...
    @Test
    fun `GIVEN a results host filter WHEN querying history THEN return the results containing mobile domains`() =
        runTest {
            val history: HistoryStorage = mock()
            val metadata: HistoryMetadataStorage = mock()
            doReturn(emptyList<HistoryMetadata>()).`when`(metadata).queryHistoryMetadata(anyString(), anyInt())
