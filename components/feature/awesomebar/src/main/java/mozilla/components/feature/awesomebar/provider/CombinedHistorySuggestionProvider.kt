...
@VisibleForTesting internal const val COMBINED_HISTORY_RESULTS_TO_FILTER_SCALE_FACTOR = 10

/**
 * A [AwesomeBar.SuggestionProvider] implementation that combines suggestions from [HistoryMetadataSuggestionProvider]
 * and [HistoryStorageSuggestionProvider]. It will return suggestions using [HistoryMetadataSuggestionProvider] first,
 * followed by suggestion from [HistoryStorageSuggestionProvider] up to the provided [maxNumberOfSuggestions].
 *
 * @param historyStorage an instance of the [HistoryStorage] used to query matching metadata records.
 * @param historyMetadataStorage an instance of the [HistoryStorage] used to query matching metadata records.
 * @param loadUrlUseCase the use case invoked to load the url when the user clicks on the suggestion.
 * @param icons optional instance of [BrowserIcons] to load fav icons for [HistoryMetadata] URLs.
...
class CombinedHistorySuggestionProvider(
    ...
    override suspend fun onInputChanged(text: String): List<AwesomeBar.Suggestion> = coroutineScope {
        ...
        val updatedMetadataSuggestions =
            if (historyTopScore != null) {
        ...
        val combinedSuggestions =
            (updatedMetadataSuggestions + historySuggestions).distinctBy { it.description }.take(maxNumberOfSuggestions)
    ...
    @VisibleForTesting fun getMaxNumberOfSuggestions() = maxNumberOfSuggestions
     ...
     */
    private suspend fun getMetadataSuggestions(query: String) =
     ...
     */
    private suspend fun getFilteredMetadataSuggestions(query: String, filter: (Uri) -> Boolean) =
        historyMetadataStorage
     ...
     * @param query String to filter history entry's title or URL by.
     */
    private suspend fun getHistorySuggestions(query: String) =
        historyStorage
     ...
     * @param filter Predicate to filter the URLs of the history entries that match the [query].
     */
    private suspend fun getFilteredHistorySuggestions(query: String, filter: (Uri) -> Boolean) =
        historyStorage
