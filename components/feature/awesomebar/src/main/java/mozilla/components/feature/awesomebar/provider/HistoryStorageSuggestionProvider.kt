...
@VisibleForTesting internal const val HISTORY_RESULTS_TO_FILTER_SCALE_FACTOR = 10

/**
 * A [AwesomeBar.SuggestionProvider] implementation that provides suggestions based on the browsing history stored in
 * the [HistoryStorage].
 *
 * @param historyStorage and instance of the [HistoryStorage] used to query matching history records.
 * @param loadUrlUseCase the use case invoked to load the url when the user clicks on the suggestion.
 * @param icons optional instance of [BrowserIcons] to load fav icons for history URLs.
...
class HistoryStorageSuggestionProvider(
     ...
     */
    private fun getHistorySuggestions(query: String) =
        historyStorage
     ...
     */
    private fun getFilteredHistorySuggestions(query: String, filter: (Uri) -> Boolean) =
        historyStorage
