...
class DownloadStorage(context: Context) {
    ...
    @VisibleForTesting internal var database: Lazy<DownloadsDatabase> = lazy { DownloadsDatabase.get(context) }
    ...
    fun getDownloadsPaged(): DataSource.Factory<Int, DownloadState> =
        downloadDao.getDownloadsPaged().map { entity ->
            entity.toDownloadState()
        }
    ...
    companion object {
        /**
         * Takes two [DownloadState] objects and the determine if they are the same, be aware this only takes into
         * considerations fields that are being stored, not all the field on [DownloadState] are stored.
         */
