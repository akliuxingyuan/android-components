 ...
 *
 * This class uses the [browserStore] to determine when private mode is active, and updates the [browserStore] whenever
 * a new search has been requested.
 *
 * NOTE: this will add [SearchRequest]s to [BrowserStore.state] when [sendSearch] is called. Client code is responsible
 * for consuming these requests and displaying something to the user.
 *
 * NOTE: client code is responsible for sending [ContentAction.ConsumeSearchRequestAction]s after consuming events. See
 * [SearchFeature] for a component that will handle this for you.
 *
