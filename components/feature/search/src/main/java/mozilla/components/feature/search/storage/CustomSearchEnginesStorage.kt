...
/**
 * A storage implementation for organizing [SearchEngine]s. Its primary use case is for persisting custom search engines
 * added by users.
 */
internal class CustomSearchEngineStorage(
    ...
    private val reader = SearchEngineReader(context, SearchEngine.Type.CUSTOM)
    private val writer = SearchEngineWriter()

    override suspend fun loadSearchEngineList(): List<SearchEngine> =
    ...
    suspend fun loadSearchEngine(identifier: String): SearchEngine =
        withContext(coroutineContext) {
            reader.loadFile(identifier, getSearchFile(identifier))
        }

    override suspend fun saveSearchEngine(searchEngine: SearchEngine): Boolean =
        withContext(coroutineContext) {
            writer.saveSearchEngineXML(searchEngine, getSearchFile(searchEngine.id))
        }

    override suspend fun removeSearchEngine(identifier: String) =
        withContext(coroutineContext) {
