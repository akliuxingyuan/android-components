...
class TrendingSearchClientTest {
    companion object {
        val GOOGLE_MOCK_RESPONSE: TrendingSearchFetcher = {
    ...
    fun `GIVEN an exception in the trending search fetcher WHEN getting trending searches THEN re-throw an IOException`() =
        runTest {
    ...
    @Test
    fun `GIVEN a search engine without a trending URL WHEN getting trending searches THEN return an empty suggestion list`() =
        runTest {
            val testSearchEngine =
                createSearchEngine(
                    name = "Test",
                    url = "https://localhost?q={searchTerms}",
                    icon = mock(),
