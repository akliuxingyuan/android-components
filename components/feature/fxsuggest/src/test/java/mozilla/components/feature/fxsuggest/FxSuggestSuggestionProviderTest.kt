...
class FxSuggestSuggestionProviderTest {
    ...
    fun inputNotEmpty() = runTest {
        ...
        assertEquals(
            testContext.resources.getString(R.string.sponsored_suggestion_description),
    ...
    fun includeNonSponsoredSuggestionsOnly() = runTest {
        ...
        suggestions.first().metadata?.let {
            ...
            val clickInfo =
                requireNotNull(
                    it[FxSuggestSuggestionProvider.MetadataKeys.CLICK_INFO] as? FxSuggestInteractionInfo.Wikipedia
                )
            assertEquals("c303282d-f2e6-46ca-a04a-35d3d873712d", clickInfo.contextId)

            val impressionInfo =
                requireNotNull(
                    it[FxSuggestSuggestionProvider.MetadataKeys.IMPRESSION_INFO] as? FxSuggestInteractionInfo.Wikipedia
                )
    ...
    fun includeSponsoredSuggestionsOnly() = runTest {
        ...
        suggestions.first().metadata?.let {
            ...
            val clickInfo =
                requireNotNull(it[FxSuggestSuggestionProvider.MetadataKeys.CLICK_INFO] as? FxSuggestInteractionInfo.Amp)
            ...
            val impressionInfo =
                requireNotNull(
                    it[FxSuggestSuggestionProvider.MetadataKeys.IMPRESSION_INFO] as? FxSuggestInteractionInfo.Amp
                )
