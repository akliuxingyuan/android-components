...
class RecentlyClosedMiddlewareTest {
    ...
    fun `closed tab storage adds normal tabs removed with TabListAction`() =
        runTest(testDispatcher) {
            ...
            val store =
                BrowserStore(
                    initialState = BrowserState(tabs = listOf(tab, tab2)),
    ...
    fun `closed tabs storage adds tabs closed one after the other without clear actions in between`() =
        runTest(testDispatcher) {
            ...
            val store = BrowserStore(middleware = listOf(UndoMiddleware(mainScope = this), middleware))
    ...
    fun `recently closed storage removes the provided tab on remove tab action`() =
        runTest(testDispatcher) {
            ...
            val store =
                BrowserStore(
                    initialState = BrowserState(closedTabs = listOf(closedTab.state)),
                    middleware = listOf(middleware),
                )
...
private suspend fun mockStorage(tabs: List<TabState> = emptyList()): RecentlyClosedMiddleware.Storage {
    val storage: RecentlyClosedMiddleware.Storage = mock()

    whenever(storage.getTabs())
        .thenReturn(
            flow {
                emit(tabs)
            }
