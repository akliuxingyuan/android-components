...
/**
 * A feature for closing tabs on this device from other devices in the [DeviceConstellation].
 *
 * This feature receives commands to close tabs using the [FxaAccountManager].
 *
 * See [CloseTabsUseCases] for the ability to close tabs that are open on other devices from this device.
 *
 * @param receiver A [CloseTabsCommandReceiver] to process the received commands.
 * @param accountManager The account manager.
 * @param owner The Android lifecycle owner for the observers. Defaults to the [ProcessLifecycleOwner].
...
class CloseTabsFeature(
    ...
    @VisibleForTesting
    internal val observer = TabsClosedEventsObserver { device, urls ->
...
internal class TabsClosedEventsObserver(internal val onTabsClosed: (Device?, List<String>) -> Unit) :
