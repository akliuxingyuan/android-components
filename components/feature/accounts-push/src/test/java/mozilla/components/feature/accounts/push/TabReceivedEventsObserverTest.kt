...
class TabReceivedEventsObserverTest {
    ...
    fun `events are delivered successfully`() {
        ...
        val events =
            listOf(AccountEvent.DeviceCommandIncoming(command = DeviceCommandIncoming.TabReceived(mock(), mock())))
        ...
        observer.onEvents(
            listOf(AccountEvent.DeviceCommandIncoming(command = DeviceCommandIncoming.TabReceived(null, mock())))
        )
